...
xqc_h3_request_t *
xqc_h3_request_create(xqc_engine_t *engine, const xqc_cid_t *cid,
                      xqc_stream_settings_t *settings, void *user_data)
{
    ...
    h3_stream =
        xqc_h3_stream_create(h3_conn, stream, XQC_H3_STREAM_TYPE_REQUEST, user_data);
    if (!h3_stream) {
    ...
    xqc_log(engine->log, XQC_LOG_DEBUG,
            "|success|stream_id:%ui|conn:%p|conn_state:%s|flag:%s|", h3_stream->stream_id,
            h3_conn->conn, xqc_conn_state_2_str(h3_conn->conn->conn_state),
...
{
    return (last_rpr_time == 0 || last_rpr_time > stream_fin_time)
               ? 0
               : xqc_calc_delay(stream_fin_time, last_rpr_time);
}
...
xqc_h3_request_destroy(xqc_h3_request_t *h3_request)
{
    ...
    xqc_log(
        h3_request->h3_stream->log, XQC_LOG_REPORT,
        "|stream_id:%ui|close_msg:%s|err:%d"
        "|rcvd_bdy_sz:%uz|snd_bdy_sz:%uz|rcvd_hdr_sz:%uz|snd_hdr_sz:%uz"
        "|create:%ui|blkd:%ui|nblkd:%ui|hdr_b:%ui|hdr_e:%ui|bdy_b:%ui|fin:%ui|recv_end:%"
        "ui"
        ...
        stats.recv_header_size, stats.send_header_size, create_time,
        ...
        xqc_calc_delay(h3_request->h3_stream->h3c->conn->conn_last_send_time,
                       create_time),
        xqc_calc_delay(h3_request->h3_stream->h3c->conn->conn_last_recv_time,
                       create_time),
        ...
        h3_request->sched_cwnd_blk_cnt, h3_request->sched_cwnd_blk_duration / 1000,
        h3_request->send_cwnd_blk_cnt, h3_request->send_cwnd_blk_duration / 1000,
        h3_request->send_pacing_blk_cnt, h3_request->send_pacing_blk_duration / 1000,
        h3s->begin_trans_state, h3s->end_trans_state, stats.is_fec_protected,
        stats.fec_recov_cnt, h3_request->block_size_mode, stats.fst_rpr_time,
        stats.last_rpr_time, stats.fec_req_delay_time, stats.extern_stream_info,
...
xqc_h3_request_close(xqc_h3_request_t *h3_request)
{
    ...
    if (ret) {
        xqc_log(conn->log, XQC_LOG_ERROR,
                "|fail|ret:%d|stream_id:%ui|conn:%p|conn_state:%s|"
                "flag:%s|",
                ret, h3s->stream_id, conn, xqc_conn_state_2_str(conn->conn_state),
                xqc_conn_flag_2_str(conn, conn->conn_flag));
        return ret;
    }

    xqc_log(conn->log, XQC_LOG_DEBUG,
            "|success|stream_id:%ui|conn:%p|conn_state:%s|flag:%s|", h3s->stream_id, conn,
...
xqc_h3_request_t *
xqc_h3_request_create_inner(xqc_h3_conn_t *h3_conn, xqc_h3_stream_t *h3_stream,
                            void *user_data)
{
    xqc_h3_request_t *h3_request;
...
xqc_h3_request_encode_rtts(xqc_h3_request_t *h3r, char *buff, size_t buff_size)
{
    ...
    for (uint32_t i = 0; i < h3_stream->paths_info_count; ++i) {
        ...
        if ((m->path_send_bytes > 0) || (m->path_recv_bytes > 0)) {
            ret = snprintf(buff + cursor, buff_size - cursor, "%" PRIu64 "-",
                           m->path_srtt / 1000);
            cursor += ret;
...
xqc_stream_info_print(xqc_h3_stream_t *h3_stream, xqc_request_stats_t *stats)
{
    ...
    ret = snprintf(buff, buff_size,
                   "(%d,%" PRIu64 ",%s,%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%u,%" PRIu64
                   ")#",
                   flag, h3_stream->recv_rate_limit, mp_settings, h3_stream->send_offset,
                   h3_stream->recv_offset, stats->cwnd_blocked_ms, stats->retrans_cnt,
                   hsk_time / 1000);
    ...
    for (uint32_t i = 0; i < h3_stream->paths_info_count; ++i) {
        ...
        if ((m->path_send_bytes > 0) || (m->path_recv_bytes > 0)) {
            ret = snprintf(buff + cursor, buff_size - cursor,
                           ...
                           "-%" PRIu64 "-%d#",
                           h3_stream->paths_info[i].path_id, m->path_pkt_send_count,
                           m->path_pkt_recv_count, m->path_send_bytes, m->path_recv_bytes,
                           m->path_srtt, m->path_app_status);
            cursor += ret;
...
#  define __calc_delay(a, b) ((a > b && b) ? (a) - (b) : 0)
    ret = snprintf(
        output, output_size,
        "%d,%u,%d,%u,%u,%" PRIu64 ",%" PRIu64 ",%s,%s,%" PRIu64 ",%" PRIu64 ",",
        stats->is_fec_protected, h3c->conn->fec_neg_fail_reason, stats->block_size_mode,
        stats->fec_recov_cnt, stats->fec_blk_lack_num, stats->fec_blk_lack_time,
...
{
    if (h3_request && settings && h3_request->h3_stream &&
        h3_request->h3_stream->stream) {
        if (xqc_stream_update_settings(h3_request->h3_stream->stream, settings) ==
            XQC_OK) {
...
xqc_h3_request_get_stats(xqc_h3_request_t *h3_request)
{
    ...
    stats.stream_err =
        conn_err != 0 ? conn_err : (int)xqc_h3_stream_get_err(h3_request->h3_stream);
    ...
    stats.cwnd_blocked_ms =
        (h3_request->sched_cwnd_blk_duration + h3_request->send_cwnd_blk_duration) / 1000;
    ...
    stats.is_fec_protected = h3_request->block_size_mode != XQC_SLIM_SIZE_REQ &&
                                     h3_request->h3_stream->h3c->conn->conn_settings
                                             .fec_params.fec_encoder_scheme != 0
                                 ? 1
                                 : 0;
    ...
    stats.fec_req_delay_time =
        xqc_get_fec_fin_delay(stats.stream_fin_time, stats.last_rpr_time);
...
xqc_h3_request_stats_print(xqc_h3_request_t *h3_request, char *str, size_t size)
{
    ...
    return snprintf(str, size,
                    ...
                    ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",cc:%" PRIu64
                    ",rtx:%u,rtt:%s",
...
xqc_int_t
xqc_h3_request_copy_header(xqc_http_header_t *dst, xqc_http_header_t *src,
                           xqc_var_buf_t *buf)
{
                            ...
                            uint8_t fin)
{
    ...
    if (NULL == lowercase_buf) {
        xqc_log(h3_request->h3_stream->log, XQC_LOG_ERROR,
                "|malloc buf for lowercase error|");
        return -XQC_EMALLOC;
    }
...
ssize_t
xqc_h3_request_send_body(xqc_h3_request_t *h3_request, unsigned char *data,
                         size_t data_size, uint8_t fin)
{
    ...
    if (sent == -XQC_EAGAIN) {
        ...
        xqc_log(
            h3_request->h3_stream->h3c->log, XQC_LOG_ERROR,
...
xqc_h3_request_recv_headers(xqc_h3_request_t *h3_request, uint8_t *fin)
{
    ...
    if (h3_request->read_flag & XQC_REQ_NOTIFY_READ_HEADER) {
        ...
        *fin = (h3_request->read_flag == XQC_REQ_NOTIFY_READ_HEADER)
                   ? h3_request->fin_flag
                   : 0;
        if (*fin) {
                         ...
                         size_t recv_buf_size, uint8_t *fin)
{
    ...
    *fin = XQC_FALSE;
    ...
    xqc_log(h3_request->h3_stream->h3c->log, XQC_LOG_DEBUG,
            "|stream_id:%ui|recv_buf_size:%z|n_recv:%z|body_recvd:%uz|body_recvd_final_"
            "size:%uz|"
            "fin:%d|conn:%p|",
            h3_request->h3_stream->stream_id, recv_buf_size, n_recv,
            h3_request->body_recvd, h3_request->body_recvd_final_size, *fin,
            h3_request->h3_stream->h3c->conn);
    return n_recv;
}
...
xqc_h3_request_on_recv_header(xqc_h3_request_t *h3r)
{
    ...
    static const xqc_request_notify_flag_t
        hdr_type_2_flag[XQC_H3_REQUEST_MAX_HEADERS_CNT] = {XQC_REQ_NOTIFY_READ_HEADER,
    ...
    if (h3r->request_if->h3_request_read_notify) {
        ret =
            h3r->request_if->h3_request_read_notify(h3r, h3r->read_flag, h3r->user_data);
        if (ret < 0) {
            xqc_log(h3r->h3_stream->log, XQC_LOG_ERROR,
                    "|h3_request_read_notify error|%d|"
                    "stream_id:%ui|conn:%p|",
...
xqc_h3_request_on_recv_body(xqc_h3_request_t *h3r)
{
    ...
    if (!xqc_list_empty(&h3r->body_buf)) {
        ...
        if (h3r->request_if->h3_request_read_notify) {
            xqc_int_t ret = h3r->request_if->h3_request_read_notify(h3r, h3r->read_flag,
                                                                    h3r->user_data);
            if (ret < 0) {
                xqc_log(h3r->h3_stream->log, XQC_LOG_ERROR,
                        "|h3_request_read_notify error|%d|"
...
xqc_h3_request_on_recv_empty_fin(xqc_h3_request_t *h3r)
{
    ...
    /*
     * if read flag is not XQC_REQ_NOTIFY_READ_NULL, it means that there is header or
     * content not received by application, shall not notify empty fin event, application
     * will be noticed when receiving header and content
     */
...
size_t
xqc_write_http_priority(xqc_h3_priority_t *prio, uint8_t *dst, size_t dstcap)
{
    uint8_t *begin = dst;
    char *fec_str = xqc_calloc(1, XQC_PRIORITY_FEC_VAL_LEN);

    size_t need = XQC_PRIORITY_URGENCY_LEN + 1 + XQC_PRIORITY_INCREMENTAL_LEN +
                  XQC_PRIORITY_SCHEDULE_LEN + 1 + XQC_PRIORITY_REINJECT_LEN + 1 +
...
xqc_h3_request_set_priority(xqc_h3_request_t *h3r, xqc_h3_priority_t *prio)
{
    ...
    if (ret != XQC_OK) {
        xqc_log(h3r->h3_stream->log, XQC_LOG_ERROR,
                "|xqc_h3_request_check_priority error|%d|stream_id:%ui|conn:%p|", ret,
                h3r->h3_stream->stream_id, h3r->h3_stream->h3c->conn);
        return ret;
    }
