...
{
    if (h3c->conn->alpn == NULL) {
        xqc_log(h3c->log, XQC_LOG_ERROR, "|alpn is null|");
        return -XQC_EFATAL;
    }
...
xqc_int_t
xqc_h3_conn_init_settings(xqc_h3_conn_t *h3c)
{
    if (h3c->conn->alpn == NULL) {
        xqc_log(h3c->log, XQC_LOG_ERROR, "|alpn is null|");
        return -XQC_EFATAL;
    ...
    h3c->local_h3_conn_settings = *local_settings;
...
xqc_h3_conn_create(xqc_connection_t *conn, void *user_data)
{
    ...
    /* avoid error handling in this function leads to crash */
    ...
    if (xqc_h3_conn_init_settings(h3c) != XQC_OK) {
        h3c->flags &= ~XQC_H3_CONN_FLAG_EXT_ENABLED;
        goto fail;
    }
    ...
    if (h3c->h3_conn_callbacks.h3_conn_init_settings) {
        ...
        xqc_log(h3c->log, XQC_LOG_DEBUG,
                ...
                "qpack_blocked_streams:%ui|"
                "qpack_compat_duplicate:%ui|",
                h3c->local_h3_conn_settings.max_field_section_size,
                h3c->local_h3_conn_settings.max_pushes,
                h3c->local_h3_conn_settings.qpack_enc_max_table_capacity,
    ...
    /*
     * read the body buffer limit. Unlike the blocked-buffer pair above it has
     * no internal default -- 0 means unbounded, which is the pre-existing
     * behaviour -- so it is copied straight through on both the client and the
     * server path.
     */
