...
ssize_t xqc_h3_stream_send_headers(xqc_h3_stream_t *h3s, xqc_http_headers_t *headers,
                                   uint8_t fin);

ssize_t xqc_h3_stream_send_data(xqc_h3_stream_t *h3s, unsigned char *data,
                                size_t data_size, uint8_t fin);

xqc_int_t xqc_h3_stream_send_finish(xqc_h3_stream_t *h3s);

xqc_int_t xqc_h3_stream_send_setting(xqc_h3_stream_t *h3s,
...
xqc_h3_stream_path_metrics_get_or_grow(xqc_h3_stream_t *h3s, uint64_t path_id)
{
    ...
    if (h3s->paths_info_count == h3s->paths_info_capacity) {
        ...
        void *nb =
            xqc_realloc(h3s->paths_info, (size_t)newcap * sizeof(*h3s->paths_info));
        if (!nb) {
            return NULL;
        }
        h3s->paths_info = nb;
