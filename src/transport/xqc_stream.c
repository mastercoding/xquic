...
xqc_stream_ready_to_write(xqc_stream_t *stream)
{
    if (!(stream->stream_flag & XQC_STREAM_FLAG_READY_TO_WRITE)) {
        if (stream->stream_encrypt_level == XQC_ENC_LEV_1RTT) {
            xqc_list_add_tail(&stream->write_stream_list,
                              &stream->stream_conn->conn_write_streams);
        }
...
xqc_stream_ready_to_read(xqc_stream_t *stream)
{
    if (!(stream->stream_flag & XQC_STREAM_FLAG_READY_TO_READ)) {
        if (stream->stream_encrypt_level == XQC_ENC_LEV_1RTT) {
            xqc_list_add_tail(&stream->read_stream_list,
                              &stream->stream_conn->conn_read_streams);
        }
...
xqc_stream_is_terminal_state(xqc_stream_t *stream)
{
    ...
    } else if ((stream->stream_type == XQC_CLI_UNI &&
                conn_type == XQC_CONN_TYPE_CLIENT) ||
               (stream->stream_type == XQC_SVR_UNI &&
...
xqc_stream_maybe_need_close(xqc_stream_t *stream)
{
    ...
    stream->stream_stats.max_pto_backoff =
        xqc_max(stream->stream_stats.max_pto_backoff,
                xqc_conn_get_max_pto_backoff(stream->stream_conn, 1));

    if (xqc_stream_is_terminal_state(stream)) {
        xqc_log(stream->stream_conn->log, XQC_LOG_DEBUG, "|stream_id:%ui|stream_type:%d|",
        ...
        xqc_list_add_tail(&stream->closing_stream_list,
                          &stream->stream_conn->conn_closing_streams);
    ...
    if (conn->conn_type == XQC_CONN_TYPE_CLIENT) {
        if (stream->stream_type == XQC_CLI_BID) {
            stream->stream_flow_ctl.fc_max_stream_data_can_send =
                remote_settings->max_stream_data_bidi_remote;
            stream->stream_flow_ctl.fc_max_stream_data_can_recv =
                local_settings->max_stream_data_bidi_local;
            ...
            stream->stream_flow_ctl.fc_max_stream_data_can_send =
                remote_settings->max_stream_data_bidi_local;
            stream->stream_flow_ctl.fc_max_stream_data_can_recv =
                local_settings->max_stream_data_bidi_remote;
            stream->stream_flow_ctl.fc_stream_recv_window_size =
                local_settings->max_stream_data_bidi_remote;

        } else {
            stream->stream_flow_ctl.fc_max_stream_data_can_send =
                remote_settings->max_stream_data_uni;
            stream->stream_flow_ctl.fc_max_stream_data_can_recv =
                local_settings->max_stream_data_uni;
            stream->stream_flow_ctl.fc_stream_recv_window_size =
                local_settings->max_stream_data_uni;
        }
...
xqc_stream_do_send_flow_ctl(xqc_stream_t *stream)
{
    ...
    xqc_log(stream->stream_conn->log, XQC_LOG_DEBUG, "|conn_flow_ctl|window:%ui|",
            stream->stream_conn->conn_flow_ctl.fc_max_data_can_send -
                stream->stream_conn->conn_flow_ctl.fc_data_sent);

    /* connection level */
    if (stream->stream_conn->conn_flow_ctl.fc_data_sent +
            stream->stream_conn->pkt_out_size >
        stream->stream_conn->conn_flow_ctl.fc_max_data_can_send) {
        xqc_log(stream->stream_conn->log, XQC_LOG_INFO,
                "|xqc_stream_send|exceed max_data:%ui|",
                stream->stream_conn->conn_flow_ctl.fc_max_data_can_send);

        stream->stream_conn->conn_flag |= XQC_CONN_FLAG_DATA_BLOCKED;
        xqc_write_data_blocked_to_packet(
            stream->stream_conn, stream->stream_conn->conn_flow_ctl.fc_max_data_can_send);
    ...
    if (stream->stream_send_offset + stream->stream_conn->pkt_out_size >
        stream->stream_flow_ctl.fc_max_stream_data_can_send) {
        xqc_log(stream->stream_conn->log, XQC_LOG_INFO,
                "|xqc_stream_send|exceed max_stream_data:%ui|",
                stream->stream_flow_ctl.fc_max_stream_data_can_send);

        stream->stream_flag |= XQC_STREAM_FLAG_DATA_BLOCKED;
        xqc_write_stream_data_blocked_to_packet(
...
xqc_stream_do_recv_flow_ctl(xqc_stream_t *stream)
{
    ...
    /* stream level */
    uint64_t available_window = stream->stream_flow_ctl.fc_max_stream_data_can_recv -
    ...
    if (available_window < stream->stream_flow_ctl.fc_stream_recv_window_size / 2) {
        if (!stream->recv_rate_bytes_per_sec) {
            if (stream->stream_flow_ctl.fc_last_window_update_time &&
                (now - stream->stream_flow_ctl.fc_last_window_update_time <
                 2 * min_srtt)) {
                stream->stream_flow_ctl.fc_stream_recv_window_size =
            ...
            stream->stream_flow_ctl.fc_stream_recv_window_size =
                stream->recv_rate_bytes_per_sec * max_srtt / 1000000;
            stream->stream_flow_ctl.fc_stream_recv_window_size =
    ...
    if (available_window < conn->conn_flow_ctl.fc_recv_windows_size / 2) {
        if (!conn->conn_settings.recv_rate_bytes_per_sec) {
            if (conn->conn_flow_ctl.fc_last_window_update_time &&
                ...
                conn->conn_flow_ctl.fc_recv_windows_size = xqc_min(
                    conn->conn_flow_ctl.fc_recv_windows_size * 2, XQC_MAX_RECV_WINDOW);
            }

            if (conn->conn_flow_ctl.fc_recv_windows_size <
                1.5 * stream->stream_flow_ctl.fc_stream_recv_window_size) {
                conn->conn_flow_ctl.fc_recv_windows_size =
                    (uint64_t)(1.5 * stream->stream_flow_ctl.fc_stream_recv_window_size);
            }
            ...
            conn->conn_flow_ctl.fc_recv_windows_size =
                conn->conn_settings.recv_rate_bytes_per_sec * max_srtt / 1000000;
            conn->conn_flow_ctl.fc_recv_windows_size =
                xqc_max(XQC_MIN_RECV_WINDOW, conn->conn_flow_ctl.fc_recv_windows_size);
            conn->conn_flow_ctl.fc_recv_windows_size =
                xqc_min(XQC_MAX_RECV_WINDOW, conn->conn_flow_ctl.fc_recv_windows_size);
            xqc_log(conn->log, XQC_LOG_DEBUG,
                    "|conn_level|fc_win_update|old_fc_win:%ui|fc_win:%ui|", old_fc_win,
                    conn->conn_flow_ctl.fc_recv_windows_size);
        }
        ...
        if (conn->conn_flow_ctl.fc_recv_windows_size > available_window) {
            ...
            xqc_log(conn->log, XQC_LOG_DEBUG,
                    ...
                    conn->conn_flow_ctl.fc_max_data_can_recv,
                    conn->conn_flow_ctl.fc_data_recved, conn->conn_flow_ctl.fc_data_read,
                    conn->conn_flow_ctl.fc_recv_windows_size);
                              ...
                              xqc_stream_type_t stream_type)
{
    if (stream_id == XQC_UNDEFINE_STREAM_ID) { /* sending part */
        if (stream_type == XQC_CLI_BID || stream_type == XQC_SVR_BID) {
            if (conn->cur_stream_id_bidi_local >=
                conn->conn_flow_ctl.fc_max_streams_bidi_can_send) {
                xqc_log(conn->log, XQC_LOG_ERROR,
                        "|exceed max_streams_bidi_can_send:%ui|",
                        conn->conn_flow_ctl.fc_max_streams_bidi_can_send);
                xqc_write_streams_blocked_to_packet(
                    conn, conn->conn_flow_ctl.fc_max_streams_bidi_can_send, 1);
                return -XQC_EPROTO;
            }

        } else {
            if (conn->cur_stream_id_uni_local >=
                conn->conn_flow_ctl.fc_max_streams_uni_can_send) {
                xqc_log(conn->log, XQC_LOG_ERROR, "|exceed max_streams_uni_can_send:%ui|",
                        conn->conn_flow_ctl.fc_max_streams_uni_can_send);
                xqc_write_streams_blocked_to_packet(
                    conn, conn->conn_flow_ctl.fc_max_streams_uni_can_send, 0);
                return -XQC_EPROTO;
            }
        }
        ...
        if (stream_type == XQC_CLI_BID || stream_type == XQC_SVR_BID) {
            if (stream_id >=
                4 * conn->conn_flow_ctl.fc_max_streams_bidi_can_recv + stream_type) {
                xqc_log(conn->log, XQC_LOG_ERROR,
                        "|exceed max_streams_bidi_can_recv:%ui|",
            ...
            if ((stream_id >> 2) >=
                conn->conn_flow_ctl.fc_max_streams_bidi_can_recv / 2) {
                conn->conn_flow_ctl.fc_max_streams_bidi_can_recv +=
                    conn->local_settings.max_streams_bidi;
                xqc_write_max_streams_to_packet(
                    conn, conn->conn_flow_ctl.fc_max_streams_bidi_can_recv, 1);
            }

        } else {
            if (stream_id >=
                4 * conn->conn_flow_ctl.fc_max_streams_uni_can_recv + stream_type) {
            ...
            if ((stream_id >> 2) >= conn->conn_flow_ctl.fc_max_streams_uni_can_recv / 2) {
                ...
                xqc_write_max_streams_to_packet(
                    conn, conn->conn_flow_ctl.fc_max_streams_uni_can_recv, 0);
            }
        }
    }
    return XQC_OK;
...
xqc_stream_t *
xqc_stream_create(xqc_engine_t *engine, const xqc_cid_t *cid,
                  xqc_stream_settings_t *settings, void *user_data)
{
    ...
    stream = xqc_create_stream_with_conn(conn, XQC_UNDEFINE_STREAM_ID, XQC_CLI_BID,
                                         settings, user_data);
    if (!stream) {
                                 ...
                                 void *user_data)
{
    ...
    /* create stream */
    return xqc_create_stream_with_conn(conn, XQC_UNDEFINE_STREAM_ID, type, NULL,
                                       user_data);
}
                            ...
                            xqc_stream_settings_t *settings, void *user_data)
{
    ...
    if (conn->conn_state >= XQC_CONN_STATE_CLOSING) {
        xqc_log(conn->log, XQC_LOG_ERROR,
                "|conn closing, cannot create stream|type:%d|state:%d|flag:%s|",
                conn->conn_type, conn->conn_state,
...
void
xqc_stream_set_priority(xqc_stream_t *stream, xqc_stream_priority_t priority)
{
    stream->stream_priority = priority;
}
...
#define __calc_delay(a, b) ((a && b && (a > b)) ? (a) - (b) : 0)
    ...
    curr_recv_delay = __calc_delay(stream->stream_stats.stream_recv_time,
                                   stream->stream_stats.create_time);
    stream->stream_conn->conn_avg_close_delay = xqc_update_avg_time(
        curr_close_delay, stream->stream_conn->conn_avg_close_delay, calculated_frames);
    stream->stream_conn->conn_latest_close_delay = curr_close_delay;
    stream->stream_conn->conn_avg_recv_delay = xqc_update_avg_time(
        curr_recv_delay, stream->stream_conn->conn_avg_recv_delay, calculated_frames);
...
#define __calc_delay(a, b) ((a && b && (a > b)) ? (a) - (b) : 0)
    ...
    xqc_log(stream->stream_conn->log, XQC_LOG_DEBUG,
            ...
            stream->stream_state_send, stream->stream_state_recv, stream->stream_id,
            stream->stream_type);
    ...
    if (ret != XQC_OK) {
        xqc_log(stream->stream_conn->log, XQC_LOG_ERROR,
                "|delete stream error|conn:%p|stream_id:%ui|ret:%d|", stream->stream_conn,
                stream->stream_id, ret);
    }

    if (xqc_id_hash_delete(stream->stream_conn->passive_streams_hash,
                           stream->stream_id) == XQC_ID_HASH_LOOP) {
        ...
        xqc_log(stream->stream_conn->log, XQC_LOG_ERROR,
                "|stream_id:%ui|hash:%ui|value:%p|node:%p|next:%p|", stream->stream_id,
                node->element.hash, node->element.value, node, node->next);
    }
    ...
    if (path_info_buff != NULL) {
        path_info_buff[0] = '\0';
        xqc_stream_path_metrics_print(stream->stream_conn, stream, path_info_buff,
                                      pi_buff_cap);
    }

    xqc_log(
        stream->stream_conn->log, XQC_LOG_STATS,
        ...
        stream->stream_conn->enable_multipath ? 1 : 0, stream->stream_state_send,
        stream->stream_state_recv, stream->stream_id, stream->stream_type,
        stream->stream_send_offset, stream->stream_data_in.next_read_offset,
        stream->stream_data_in.merged_offset_end, stream->stream_data_in.stream_length,
        stream->stream_stats.create_time,
        __calc_delay(stream->stream_stats.first_write_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.first_snd_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.local_fin_write_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.local_fin_snd_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.peer_fin_rcv_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.peer_fin_read_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.all_data_acked_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.first_fin_ack_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.close_time, stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.app_reset_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.local_reset_time,
                     stream->stream_stats.create_time),
        __calc_delay(stream->stream_stats.peer_reset_time,
                     stream->stream_stats.create_time),
        xqc_conn_addr_str(stream->stream_conn), path_info_buff ? path_info_buff : "",
        (stream->stream_stats.recv_time_with_fec == 0 ||
         (stream->stream_stats.final_packet_time <
          stream->stream_stats.recv_time_with_fec))
            ? 0
            : (stream->stream_stats.final_packet_time -
               stream->stream_stats.recv_time_with_fec),
        stream->stream_stats.fec_send_rpr_cnt, stream->stream_stats.retrans_pkt_cnt);
#undef __calc_delay
...
xqc_stream_record_trans_state(xqc_stream_t *stream, xqc_bool_t begin)
{
    ...
    if (begin && stream->begin_trans_state[0] == 0) {
        xqc_conn_encode_transport_state(conn, stream->begin_trans_state,
                                        XQC_STREAM_TRANSPORT_STATE_SZ);
    }

    if (!begin && stream->end_trans_state[0] == 0) {
        xqc_conn_encode_transport_state(conn, stream->end_trans_state,
                                        XQC_STREAM_TRANSPORT_STATE_SZ);
    }
}
...
xqc_stream_close_with_error(xqc_stream_t *stream, uint64_t err_code)
{
    ...
    stream->stream_stats.max_pto_backoff = xqc_max(stream->stream_stats.max_pto_backoff,
...
xqc_int_t
xqc_insert_passive_stream_hash(xqc_connection_t *conn, int64_t cur_max_sid,
                          ...
                          void *user_data)
{
    ...
    if (xqc_stream_is_bidi(stream_id) && sid > conn->max_stream_id_bidi_remote) {
        ...
        if (!xqc_id_hash_find(conn->passive_streams_hash, stream_id)) {
            /* already closed */
            xqc_log(conn->log, XQC_LOG_DEBUG, "|stream already closed|stream_id:%ui|",
                    stream_id);
            return NULL;
        }
    }
    ...
    xqc_stream_t *stream =
        xqc_create_stream_with_conn(conn, stream_id, 0, NULL, user_data);
    if (stream == NULL) {
        xqc_log(conn->log, XQC_LOG_ERROR,
...
xqc_int_t
xqc_stream_update_settings(xqc_stream_t *stream, xqc_stream_settings_t *settings)
{
    xqc_connection_t *conn = NULL;
    xqc_usec_t max_srtt = 0;
    uint64_t old_fc_win = 0, new_offset = 0;

    if (stream && settings && settings->recv_rate_bytes_per_sec) {
        ...
        if (conn->conn_settings.enable_stream_rate_limit) {
            ...
            if (new_offset > stream->stream_flow_ctl.fc_max_stream_data_can_recv) {
                ...
                xqc_log(conn->log, XQC_LOG_DEBUG,
                        "|new_max_data:%ui|stream_max_recv_offset:%ui|next_read_offset:%"
                        "ui|window_size:%ui|",
                        stream->stream_flow_ctl.fc_max_stream_data_can_recv,
                        stream->stream_max_recv_offset,
                        stream->stream_data_in.next_read_offset,
                ...
                xqc_write_max_stream_data_to_packet(
                    conn, stream->stream_id,
                    stream->stream_flow_ctl.fc_max_stream_data_can_recv, XQC_PTYPE_NUM);
            }
            return XQC_OK;
        }
    }
...
xqc_read_crypto_stream(xqc_stream_t *stream)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG, "|xqc_read_crypto_stream|level:%d|",
            stream->stream_encrypt_level);
    ...
    xqc_list_for_each_safe(pos, next, &stream->stream_data_in.frames_tailq)
    {
        ...
        size_t data_len = stream_frame->data_offset + stream_frame->data_length -
                          stream->stream_data_in.next_read_offset;
        unsigned char *data_start =
            stream_frame->data +
            (stream->stream_data_in.next_read_offset - stream_frame->data_offset);

        stream->stream_data_in.next_read_offset =
            stream->stream_data_in.next_read_offset + data_len;

        xqc_int_t ret = xqc_tls_process_crypto_data(
            conn->tls, stream->stream_encrypt_level, data_start, data_len);
        ...
        if (ret != XQC_OK) {
            xqc_log(stream->stream_conn->log, XQC_LOG_ERROR,
                    "|xqc_tls_process_crypto_data error: %d|", ret);
...
xqc_crypto_stream_on_read(xqc_stream_t *stream, void *user_data)
{
    ...
    if (encrypt_level == XQC_ENC_LEV_INIT) {
        ...
        switch (cur_state) {
        case XQC_CONN_STATE_ESTABED: next_state = XQC_CONN_STATE_ESTABED; break;
    ...
    xqc_log(stream->stream_conn->log, XQC_LOG_DEBUG,
            "|encrypt_level:%d|cur_state:%s|next_state:%s|", encrypt_level,
            xqc_conn_state_2_str(cur_state), xqc_conn_state_2_str(next_state));
    return 0;
}
...
int
xqc_crypto_stream_send(xqc_stream_t *stream, xqc_list_head_t *crypto_data_list,
    ...
    xqc_list_for_each_safe(pos, next, head)
    {
        ...
        if (buf->data_len > 0) {
            ...
            while (stream->stream_send_offset < send_data_num) {
                unsigned int header_size = xqc_crypto_frame_header_size(
                    stream->stream_send_offset, buf->data_len - offset);
...
xqc_crypto_stream_on_write(xqc_stream_t *stream, void *user_data)
{
    ...
    if (encrypt_level == XQC_ENC_LEV_INIT) {
        ...
        case XQC_CONN_STATE_SERVER_INITIAL_RECVD:
            ...
            xqc_log(stream->stream_conn->log, XQC_LOG_DEBUG, "|cur_state:%d|switch|",
                    cur_state);
...
xqc_stream_t *
xqc_create_crypto_stream(xqc_connection_t *conn, xqc_encrypt_level_t encrypt_level,
                         void *user_data)
{
    xqc_log(conn->log, XQC_LOG_DEBUG, "|encrypt_level:%d|cur_state:%s|", encrypt_level,
    ...
    stream->stream_type =
        conn->conn_type == XQC_CONN_TYPE_CLIENT ? XQC_CLI_BID : XQC_SVR_BID;
...
{
    xqc_log(conn->log, XQC_LOG_DEBUG,
            "|destroy crypto stream|encrypt_level:%d"
            "|cur_state:%s|",
            stream->stream_encrypt_level, xqc_conn_state_2_str(conn->conn_state));
...
ssize_t
xqc_stream_recv(xqc_stream_t *stream, unsigned char *recv_buf, size_t recv_buf_size,
                uint8_t *fin)
{
    xqc_list_head_t *pos, *next;
    xqc_stream_frame_t *stream_frame = NULL;
    ...
    *fin = 0;
    ...
    xqc_list_for_each_safe(pos, next, &stream->stream_data_in.frames_tailq)
    {
        ...
        if (stream_frame->data_offset < stream->stream_data_in.next_read_offset) {
            uint64_t offset =
                stream->stream_data_in.next_read_offset - stream_frame->data_offset;
            stream_frame->next_read_offset =
                xqc_max(stream_frame->next_read_offset, offset);
        }

        frame_left = stream_frame->data_length - stream_frame->next_read_offset;

        if (read + frame_left <= recv_buf_size) {
            memcpy(recv_buf + read, stream_frame->data + stream_frame->next_read_offset,
                   frame_left);
    ...
    if (ret) {
        xqc_log(stream->stream_conn->log, XQC_LOG_ERROR,
                "|xqc_stream_do_recv_flow_ctl error|stream_id:%ui|", stream->stream_id);
        return ret;
    }
...
ssize_t
xqc_stream_send(xqc_stream_t *stream, unsigned char *send_data, size_t send_data_size,
                uint8_t fin)
{
    xqc_connection_t *conn = stream->stream_conn;
    if (conn->conn_state >= XQC_CONN_STATE_CLOSING) {
        xqc_conn_log(conn, XQC_LOG_INFO, "|conn closing, cannot send|stream_id:%ui|",
                     stream->stream_id);
        xqc_stream_shutdown_write(stream);
        return -XQC_CLOSING;
    }
    if (stream->stream_state_send >= XQC_SEND_STREAM_ST_RESET_SENT) {
        xqc_conn_log(conn, XQC_LOG_INFO, "|stream reset sent, cannot send|stream_id:%ui|",
                     stream->stream_id);
        xqc_stream_shutdown_write(stream);
        return -XQC_ESTREAM_RESET;
    }
    if (stream->stream_flag & XQC_STREAM_FLAG_FIN_WRITE) {
        xqc_conn_log(conn, XQC_LOG_WARN, "|fin write, cannot send|stream_id:%ui|",
                     stream->stream_id);
        xqc_stream_shutdown_write(stream);
        return 0;
    }
    ...
    while (offset < send_data_size || fin_only) {
        ...
        if (!xqc_send_queue_can_write(conn->conn_send_queue)) {
            ...
            xqc_log(conn->log, XQC_LOG_DEBUG,
                    "|too many packets used|sndq_packets_used:%ud|",
        ...
        if (pkt_type == XQC_PTYPE_0RTT &&
            ...
            xqc_log(conn->log, XQC_LOG_DEBUG,
                    "|too many 0rtt packets|zero_rtt_count:%ud|", conn->zero_rtt_count);
            ret = -XQC_EAGAIN;
            goto do_buff;
        }
        ...
        ret = xqc_write_stream_frame_to_packet(
            conn, stream, pkt_type, fin, send_data + offset, send_data_size - offset,
            &send_data_written);
        if (ret) {
...
do_buff:
    ...
    if ((!conn->first_data_send_time) &&
        ((stream->stream_type == XQC_CLI_BID) || (stream->stream_type == XQC_SVR_BID))) {
    ...
    xqc_log(conn->log, XQC_LOG_INFO,
            ...
            "send_data_size:%uz|offset:%uz|fin:%d|stream_flag:%d|conn:%p|conn_state:%s|"
            "flag:%s|",
            ret, stream->stream_id, stream->stream_send_offset,
            xqc_pkt_type_2_str(pkt_type), buff_1rtt, send_data_size, offset, fin,
            stream->stream_flag, conn, xqc_conn_state_2_str(conn->conn_state),
            xqc_conn_flag_2_str(conn, conn->conn_flag));
    xqc_log_event(conn->log, TRA_STREAM_DATA_MOVED, stream, 0, send_data_size, 0, fin,
                  ret, pkt_type, buff_1rtt, offset);
...
ssize_t
xqc_stream_buff_data(xqc_stream_t *stream, unsigned char *send_data,
...
xqc_stream_write_buffed_data_to_packets(xqc_stream_t *stream)
{
    ...
    xqc_list_for_each_safe(pos, next, &buff_list->write_buff_list)
    {
        ...
        while (offset < send_data_size || fin_only) {
            ...
            if (ret) {
                xqc_log(conn->log, XQC_LOG_ERROR,
                        "|xqc_write_stream_frame_to_packet error|");
                return ret;
            }
            offset += send_data_written;
            xqc_log(conn->log, XQC_LOG_DEBUG,
...
xqc_process_write_streams(xqc_connection_t *conn)
{
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_write_streams)
    {
        ...
        xqc_log(conn->log, XQC_LOG_DEBUG,
                "|stream_write_notify|flag:%d|stream_id:%ui|conn:%p|cnt:%d|",
                stream->stream_flag, stream->stream_id, stream->stream_conn, cnt++);
        if (stream->stream_if->stream_write_notify == NULL) {
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|stream_write_notify is NULL|flag:%d|stream_id:%ui|conn:%p|",
        ...
        if (ret < 0) {
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|stream_write_notify err:%d|flag:%d|stream_id:%ui|conn:%p|", ret,
                    stream->stream_flag, stream->stream_id, stream->stream_conn);
            XQC_CONN_ERR(conn, TRA_INTERNAL_ERROR);
        }
    }
...
xqc_process_read_streams(xqc_connection_t *conn)
{
    ...
    {
        if (pos->next == pos) {
            xqc_log(conn->log, XQC_LOG_FATAL, "|pos:%p|conn:%p|", pos, conn);
            XQC_CONN_ERR(conn, TRA_INTERNAL_ERROR);
            return;
        }
        stream = xqc_list_entry(pos, xqc_stream_t, read_stream_list);
        xqc_log(conn->log, XQC_LOG_DEBUG,
                "|stream_read_notify|flag:%d|stream_id:%ui|conn:%p|", stream->stream_flag,
                stream->stream_id, stream->stream_conn);
        if (stream->stream_if->stream_read_notify == NULL) {
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|stream_read_notify is NULL|flag:%d|stream_id:%ui|conn:%p|",
                    stream->stream_flag, stream->stream_id, stream->stream_conn);
            XQC_CONN_ERR(conn, TRA_INTERNAL_ERROR);
            return;
        }
        ...
        if (ret < 0) {
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|stream_read_notify err:%d|flag:%d|stream_id:%ui|conn:%p|", ret,
                    stream->stream_flag, stream->stream_id, stream->stream_conn);
            XQC_CONN_ERR(conn, TRA_INTERNAL_ERROR);
        }
    }
...
{
    xqc_log_event(stream->stream_conn->log, TRA_STREAM_STATE_UPDATED, stream,
                  XQC_LOG_STREAM_SEND, state);
...
{
    return xqc_stream_is_uni(strm->stream_id) ? XQC_STREAM_UNI : XQC_STREAM_BIDI;
}
...
xqc_stream_closing(xqc_stream_t *stream, xqc_int_t err)
{
    if (stream->stream_if->stream_closing_notify) {
        stream->stream_if->stream_closing_notify(stream, err, stream->user_data);
    }
}
