...
xqc_conn_settings_t internal_default_conn_settings = {
    ...
    .enable_multipath = 0,
    .multipath_version = XQC_MULTIPATH_3E, /* draft-21 default; dispatch still accepts
                                              XQC_MULTIPATH_10 for backward interop */
    ...
    .scheduler_params = {.bw_Bps_thr = 375000,
...
static void
xqc_conn_dgram_probe_timeout(xqc_gp_timer_id_t gp_timer_id, xqc_usec_t now,
                             void *user_data)
{
    ...
    if (conn->last_dgram && conn->last_dgram->data_len != 0) {
        ...
        ret = xqc_datagram_send(conn, conn->last_dgram->data, probe_size, NULL,
                                XQC_DATA_QOS_PROBING);
        xqc_log(conn->log, XQC_LOG_DEBUG, "|timer_based_dgram_probe|ret:%d|dgram_sz:%z|",
                ret, probe_size);
    }
}

void
xqc_conn_set_default_sched_params(xqc_conn_settings_t *src_settings,
                                  xqc_conn_settings_t *settings)
{
    ...
    if (settings->scheduler_params.loss_percent_thr_high == 0) {
        settings->scheduler_params.loss_percent_thr_high =
            src_settings->scheduler_params.loss_percent_thr_high;
    }

    if (settings->scheduler_params.loss_percent_thr_low == 0) {
        settings->scheduler_params.loss_percent_thr_low =
            src_settings->scheduler_params.loss_percent_thr_low;
    }

    if (settings->scheduler_params.pto_cnt_thr == 0) {
        settings->scheduler_params.pto_cnt_thr =
            src_settings->scheduler_params.pto_cnt_thr;
    }

    if (settings->scheduler_params.rtt_us_thr_high == 0) {
        settings->scheduler_params.rtt_us_thr_high =
            src_settings->scheduler_params.rtt_us_thr_high;
    }

    if (settings->scheduler_params.rtt_us_thr_low == 0) {
        settings->scheduler_params.rtt_us_thr_low =
            src_settings->scheduler_params.rtt_us_thr_low;
    }
}
...
xqc_server_set_conn_settings(xqc_engine_t *engine, const xqc_conn_settings_t *settings)
{
    ...
    engine->default_conn_settings.datagram_force_retrans_on =
        settings->datagram_force_retrans_on;
    ...
    engine->default_conn_settings.recv_rate_bytes_per_sec =
...
#endif
    engine->default_conn_settings.adaptive_ack_frequency =
        settings->adaptive_ack_frequency;

    engine->default_conn_settings.scheduler_params = settings->scheduler_params;
    xqc_conn_set_default_sched_params(&internal_default_conn_settings,
                                      &engine->default_conn_settings);

    if (settings->max_udp_payload_size != 0) {
        engine->default_conn_settings.max_udp_payload_size =
            settings->max_udp_payload_size;
    }

    if (engine->default_conn_settings.init_recv_window) {
        engine->default_conn_settings.init_recv_window =
            xqc_max(engine->default_conn_settings.init_recv_window, XQC_QUIC_MAX_MSS);
    }
    ...
    if (settings->max_ack_delay) {
        engine->default_conn_settings.max_ack_delay =
            xqc_min(settings->max_ack_delay, XQC_DEFAULT_MAX_ACK_DELAY);
    }

    if (settings->datagram_redundant_probe) {
        engine->default_conn_settings.datagram_redundant_probe =
            xqc_max(settings->datagram_redundant_probe,
    ...
    if (settings->fec_conn_queue_rpr_timeout > 0) {
        engine->default_conn_settings.fec_conn_queue_rpr_timeout =
            settings->fec_conn_queue_rpr_timeout;
    }
    ...
    engine->default_conn_settings.keyupdate_pkt_threshold =
        settings->keyupdate_pkt_threshold;
    engine->default_conn_settings.max_datagram_frame_size =
        settings->max_datagram_frame_size;
    ...
    engine->default_conn_settings.max_path_id_grant_max_value =
        settings->max_path_id_grant_max_value;
    ...
    if (engine->default_conn_settings.enable_encode_fec) {
        xqc_set_fec_schemes(
            settings->fec_params.fec_encoder_schemes,
            settings->fec_params.fec_encoder_schemes_num,
            engine->default_conn_settings.fec_params.fec_encoder_schemes,
            &engine->default_conn_settings.fec_params.fec_encoder_schemes_num);
        /* if no fec scheme was negotiated succesfully, set enable_encode_fec to 0 */
        engine->default_conn_settings.enable_encode_fec =
            engine->default_conn_settings.fec_params.fec_encoder_schemes_num == 0
                ? 0
                : settings->enable_encode_fec;
        if (settings->fec_params.fec_code_rate) {
            engine->default_conn_settings.fec_params.fec_code_rate =
                settings->fec_params.fec_code_rate;
        }
        if (settings->fec_params.fec_max_symbol_num_per_block) {
            engine->default_conn_settings.fec_params.fec_max_symbol_num_per_block =
                settings->fec_params.fec_max_symbol_num_per_block;
        }
        if (settings->fec_params.fec_mp_mode) {
            engine->default_conn_settings.fec_params.fec_mp_mode =
                settings->fec_params.fec_mp_mode;
        }
    ...
    if (engine->default_conn_settings.enable_decode_fec) {
        xqc_set_fec_schemes(
            settings->fec_params.fec_decoder_schemes,
            settings->fec_params.fec_decoder_schemes_num,
            engine->default_conn_settings.fec_params.fec_decoder_schemes,
            &engine->default_conn_settings.fec_params.fec_decoder_schemes_num);
        /* if no fec scheme was negotiated succesfully, set enable_decode_fec to 0 */
        engine->default_conn_settings.enable_decode_fec =
            engine->default_conn_settings.fec_params.fec_decoder_schemes_num == 0
                ? 0
                : settings->enable_decode_fec;
        if (settings->fec_params.fec_max_window_size) {
            engine->default_conn_settings.fec_params.fec_max_window_size =
                xqc_min(settings->fec_params.fec_max_window_size, XQC_SYMBOL_CACHE_LEN);
        }
        if (settings->fec_params.fec_blk_log_mod) {
            engine->default_conn_settings.fec_params.fec_blk_log_mod =
                settings->fec_params.fec_blk_log_mod;
        }
        if (settings->fec_params.fec_log_on) {
            engine->default_conn_settings.fec_params.fec_log_on =
                settings->fec_params.fec_log_on;
        }
        if (settings->fec_params.fec_packet_mask_mode) {
            engine->default_conn_settings.fec_params.fec_packet_mask_mode =
                settings->fec_params.fec_packet_mask_mode;
        }
    }
    ...
    if ((settings->extended_ack_features & XQC_ACK_EXT_FEATURE_BIT_RECV_TS) &&
         ...
         * Currently, we only use receive_timestamps features, so set
         * extended_ack_features = XQC_ACK_EXT_FEATURE_BIT_RECV_TS.
         *
         * Besides, multipath-quic and ack_ext are incompatible right now, disable ack_ext
         * when enable multipath.
         * TODO: coordinate them in the future.
         */
        engine->default_conn_settings.extended_ack_features =
            XQC_ACK_EXT_FEATURE_BIT_RECV_TS;
        engine->default_conn_settings.max_receive_timestamps_per_ack =
            settings->max_receive_timestamps_per_ack;
        engine->default_conn_settings.receive_timestamps_exponent =
    ...
    if (settings->loss_detection_pkt_thresh > 0) {
        engine->default_conn_settings.loss_detection_pkt_thresh =
            settings->loss_detection_pkt_thresh;
    }

    if (settings->reinj_flexible_deadline_srtt_factor > 0) {
        engine->default_conn_settings.reinj_flexible_deadline_srtt_factor =
            settings->reinj_flexible_deadline_srtt_factor;
    }
    ...
    if (settings->reinj_deadline_lower_bound > 0) {
        engine->default_conn_settings.reinj_deadline_lower_bound =
            settings->reinj_deadline_lower_bound;
    }

    if (settings->standby_path_probe_timeout > 0) {
        /* no less than 500ms */
        engine->default_conn_settings.standby_path_probe_timeout =
            xqc_max(settings->standby_path_probe_timeout, XQC_MIN_STANDBY_RPOBE_TIMEOUT);
    }
    ...
    if (settings->probing_pkt_out_size > 0) {
        engine->default_conn_settings.probing_pkt_out_size =
            settings->probing_pkt_out_size;
    }
    ...
    if (settings->max_blocked_buf_per_stream > 0) {
        engine->default_conn_settings.max_blocked_buf_per_stream =
            settings->max_blocked_buf_per_stream;
    } else {
        engine->default_conn_settings.max_blocked_buf_per_stream =
            XQC_H3_STREAM_MAX_BLOCKED_BUF_SIZE_DEFAULT;
    }

    if (settings->max_blocked_buf_per_conn > 0) {
        engine->default_conn_settings.max_blocked_buf_per_conn =
            settings->max_blocked_buf_per_conn;
    } else {
        engine->default_conn_settings.max_blocked_buf_per_conn =
            XQC_H3_CONN_MAX_BLOCKED_BUF_SIZE_DEFAULT;
    }
...
xqc_conn_flag_2_str(xqc_connection_t *conn, xqc_conn_flag_t conn_flag)
{
    ...
    for (int i = 0; i < XQC_CONN_FLAG_SHIFT_NUM; i++) {
        if (conn_flag & 1ULL << i) {
            wsize = snprintf(engine->conn_flag_str_buf + pos,
                             sizeof(engine->conn_flag_str_buf) - pos, "%s ",
...
xqc_conn_init_trans_settings(xqc_connection_t *conn)
{
    ...
    if (conn->conn_settings.is_interop_mode) {
        ...
        if (conn->conn_settings.recv_rate_bytes_per_sec) {
            ls->max_data =
                conn->conn_settings.recv_rate_bytes_per_sec * XQC_FC_INIT_RTT / 1000000;
    ...
    for (i = 0, co_bytes = 0; i < XQC_CO_STR_MAX_LEN; i++) {
        if (co_bytes == 4) {
            if ((co_str[i] != ',' && co_str[i] != '\0')) {
                ...
                ls->conn_options[ls->conn_option_num++] = XQC_CO_TAG(
                    co_str[i - 4], co_str[i - 3], co_str[i - 2], co_str[i - 1]);
            }
    ...
    if (conn->conn_settings.enable_encode_fec) {
        ...
        ls->fec_max_symbols_num =
            conn->conn_settings.fec_params.fec_max_symbol_num_per_block;
        ls->fec_encoder_schemes_num =
            conn->conn_settings.fec_params.fec_encoder_schemes_num;
        for (xqc_int_t i = 0; i < conn->conn_settings.fec_params.fec_encoder_schemes_num;
             i++) {
            ls->fec_encoder_schemes[i] =
                conn->conn_settings.fec_params.fec_encoder_schemes[i];
        }
    }
    if (conn->conn_settings.enable_decode_fec) {
        ls->enable_decode_fec = conn->conn_settings.enable_decode_fec;
        ls->fec_decoder_schemes_num =
            conn->conn_settings.fec_params.fec_decoder_schemes_num;
        for (xqc_int_t i = 0; i < conn->conn_settings.fec_params.fec_decoder_schemes_num;
             i++) {
            ls->fec_decoder_schemes[i] =
                conn->conn_settings.fec_params.fec_decoder_schemes[i];
        }
    }
#endif
}
...
xqc_conn_init_flow_ctl(xqc_connection_t *conn)
{
    ...
    flow_ctl->fc_max_data_can_send =
        settings->max_data; /* replace with the value specified by peer after handshake */
    flow_ctl->fc_max_data_can_recv = settings->max_data;
    flow_ctl->fc_max_streams_bidi_can_send =
        settings->max_streams_bidi; /* replace with the value specified by peer after
                                       handshake */
    flow_ctl->fc_max_streams_bidi_can_recv = settings->max_streams_bidi;
    flow_ctl->fc_max_streams_uni_can_send =
        settings->max_streams_uni; /* replace with the value specified by peer after
                                      handshake */
...
xqc_conn_init_timer_manager(xqc_connection_t *conn)
{
    ...
    xqc_timer_set(timer_manager, XQC_TIMER_CONN_IDLE, now,
                  xqc_conn_get_idle_timeout(conn) * 1000);
...
static void
xqc_conn_set_ack_ext_local_settings(xqc_trans_settings_t *local_settings,
                                    const xqc_conn_settings_t *settings)
{
    if (settings->extended_ack_features & XQC_ACK_EXT_FEATURE_BIT_RECV_TS) {
        local_settings->extended_ack_features = 2;
        local_settings->max_receive_timestamps_per_ack =
            settings->max_receive_timestamps_per_ack;
        local_settings->receive_timestamps_exponent =
            settings->receive_timestamps_exponent;
    }
}
...
xqc_conn_create(xqc_engine_t *engine, xqc_cid_t *dcid, xqc_cid_t *scid,
                const xqc_conn_settings_t *settings, void *user_data,
                xqc_conn_type_t type)
{
    xqc_connection_t *xc = NULL;
#ifdef XQC_PROTECT_POOL_MEM
    xqc_memory_pool_t *pool =
        xqc_create_pool(engine->config->conn_pool_size, settings->protect_pool_mem);
#else
    ...
    xqc_memcpy(xc->conn_settings.conn_option_str, settings->conn_option_str,
               XQC_CO_STR_MAX_LEN);
    ...
    if (xc->conn_settings.max_udp_payload_size == 0) {
        xc->conn_settings.max_udp_payload_size =
            engine->default_conn_settings.max_udp_payload_size;
    }
    ...
    if (xc->conn_settings.datagram_redundant_probe) {
        xc->conn_settings.datagram_redundant_probe =
            xqc_max(xc->conn_settings.datagram_redundant_probe,
                    XQC_MIN_DATAGRAM_REDUNDANT_PROBE_INTERVAL);
    }
    ...
    if (xc->conn_settings.init_recv_window) {
        xc->conn_settings.init_recv_window =
            xqc_max(xc->conn_settings.init_recv_window, XQC_QUIC_MAX_MSS);
    ...
    if (xc->conn_settings.standby_path_probe_timeout) {
        xc->conn_settings.standby_path_probe_timeout = xqc_max(
            xc->conn_settings.standby_path_probe_timeout, XQC_MIN_STANDBY_RPOBE_TIMEOUT);
    }

    if (xc->conn_settings.max_pkt_out_size == 0) {
        xc->conn_settings.max_pkt_out_size =
    ...
    if (xc->conn_settings.pto_backoff_factor == 0) {
        xc->conn_settings.pto_backoff_factor =
            engine->default_conn_settings.pto_backoff_factor;
    }

    if (xc->conn_settings.loss_detection_pkt_thresh == 0) {
        xc->conn_settings.loss_detection_pkt_thresh =
            engine->default_conn_settings.loss_detection_pkt_thresh;
    }

    xc->version =
        (type == XQC_CONN_TYPE_CLIENT) ? settings->proto_version : XQC_IDRAFT_INIT_VER;
    ...
    if (xc->conn_settings.reinj_flexible_deadline_srtt_factor == 0) {
        xc->conn_settings.reinj_flexible_deadline_srtt_factor =
            engine->default_conn_settings.reinj_flexible_deadline_srtt_factor;
    }

    if (xc->conn_settings.reinj_hard_deadline == 0) {
        xc->conn_settings.reinj_hard_deadline =
            engine->default_conn_settings.reinj_hard_deadline;
    }

    if (xqc_conn_is_current_mp_version_supported(xc->conn_settings.multipath_version) !=
        XQC_OK) {
    ...
    if (xc->conn_settings.probing_pkt_out_size == 0) {
        xc->conn_settings.probing_pkt_out_size =
            engine->default_conn_settings.probing_pkt_out_size;
    }
    ...
    if (xc->conn_settings.enable_encode_fec) {
        if (xc->conn_settings.fec_params.fec_code_rate == 0) {
            xc->conn_settings.fec_params.fec_code_rate =
                engine->default_conn_settings.fec_params.fec_code_rate;
        }
        if (xc->conn_settings.fec_params.fec_ele_bit_size == 0) {
            xc->conn_settings.fec_params.fec_ele_bit_size =
                engine->default_conn_settings.fec_params.fec_ele_bit_size;
        }
        if (xc->conn_settings.fec_params.fec_protected_frames == 0) {
            xc->conn_settings.fec_params.fec_protected_frames =
                engine->default_conn_settings.fec_params.fec_protected_frames;
        }
        if (xc->conn_settings.fec_params.fec_max_symbol_num_per_block == 0) {
            xc->conn_settings.fec_params.fec_max_symbol_num_per_block =
                engine->default_conn_settings.fec_params.fec_max_symbol_num_per_block;
        }
        if (xc->conn_settings.fec_params.fec_mp_mode == 0) {
            xc->conn_settings.fec_params.fec_mp_mode =
                engine->default_conn_settings.fec_params.fec_mp_mode;
        }
    ...
    if (xc->conn_settings.enable_decode_fec) {
        if (xc->conn_settings.fec_params.fec_max_window_size) {
            xc->conn_settings.fec_params.fec_max_window_size = xqc_min(
                xc->conn_settings.fec_params.fec_max_window_size, XQC_SYMBOL_CACHE_LEN);

        } else {
            xc->conn_settings.fec_params.fec_max_window_size =
                engine->default_conn_settings.fec_params.fec_max_window_size;
        }
        if (xc->conn_settings.fec_params.fec_blk_log_mod == 0) {
            xc->conn_settings.fec_params.fec_blk_log_mod =
                engine->default_conn_settings.fec_params.fec_blk_log_mod;
        }
        if (xc->conn_settings.fec_params.fec_log_on == 0) {
            xc->conn_settings.fec_params.fec_log_on =
                engine->default_conn_settings.fec_params.fec_log_on;
        }

        if (xc->conn_settings.fec_params.fec_packet_mask_mode == 0) {
            xc->conn_settings.fec_params.fec_packet_mask_mode =
                engine->default_conn_settings.fec_params.fec_packet_mask_mode;
        }
    }
    if (xc->conn_settings.fec_conn_queue_rpr_timeout == 0) {
        xc->conn_settings.fec_conn_queue_rpr_timeout =
            engine->default_conn_settings.fec_conn_queue_rpr_timeout;
    }

    if (xc->conn_settings.enable_encode_fec || xc->conn_settings.enable_decode_fec) {
    ...
    xqc_cid_set_set_largest_seq_or_rpt(&xc->scid_set, XQC_INITIAL_PATH_ID,
                                       scid->cid_seq_num);
    xqc_cid_copy(&(xc->initial_scid), scid);

    xc->engine = engine;
    xc->log = xqc_log_init(engine->log->log_level, engine->log->log_event,
                           engine->log->qlog_importance, engine->log->log_timestamp,
                           engine->log->log_level_name, engine,
                           engine->log->log_callbacks, engine->log->user_data);
    xc->log->scid = xc->scid_set.original_scid_str;
    xc->transport_cbs = engine->transport_cbs;
    xc->user_data = user_data;
    xc->conn_type = type;
    xc->conn_flag = 0;
    xc->conn_state = (type == XQC_CONN_TYPE_SERVER) ? XQC_CONN_STATE_SERVER_INIT
    ...
    if (xc->conn_settings.datagram_redundant_probe) {
        xc->last_dgram = xqc_var_buf_create_with_limit(XQC_MAX_PACKET_OUT_SIZE,
                                                       XQC_MAX_PACKET_OUT_SIZE);
        if (xc->last_dgram == NULL) {
            goto fail;
        }

        xc->dgram_probe_timer = xqc_conn_register_gp_timer(
            xc, "dgram_probe", xqc_conn_dgram_probe_timeout, xc);
    ...
    if (xqc_id_hash_init(xc->streams_hash, xqc_default_allocator,
    ...
    if (xqc_insert_conns_hash(engine->conns_hash, xc, xc->scid_set.user_scid.cid_buf,
    ...
    xc->scheduler =
        xqc_pcalloc(xc->conn_pool, xc->scheduler_callback->xqc_scheduler_size());
    if (xc->scheduler == NULL) {
        goto fail;
    }
    xc->scheduler_callback->xqc_scheduler_init(xc->scheduler, xc->log,
                                               &xc->conn_settings.scheduler_params);
    ...
    if (xc->conn_settings.reinj_ctl_callback.xqc_reinj_ctl_init) {
        ...
        xc->reinj_ctl =
            xqc_pcalloc(xc->conn_pool, xc->reinj_callback->xqc_reinj_ctl_size());
    ...
    xqc_log(xc->log, XQC_LOG_DEBUG, "|success|scid:%s|dcid:%s|conn:%p|",
            xqc_scid_str(engine, &xc->scid_set.user_scid),
            xqc_dcid_str(engine, &xc->dcid_set.current_dcid), xc);
...
xqc_int_t
xqc_conn_encode_local_tp(xqc_connection_t *conn, uint8_t *dst, size_t dst_cap,
                         size_t *dst_len)
{
...
xqc_conn_create_server_tls(xqc_connection_t *conn)
{
    ...
    ret = xqc_conn_encode_local_tp(
        conn, cfg.trans_params, XQC_MAX_TRANSPORT_PARAM_BUF_LEN, &cfg.trans_params_len);
    if (ret != XQC_OK) {
        return ret;
    }
...
xqc_conn_server_create(xqc_engine_t *engine, const struct sockaddr *local_addr,
                       ...
                       socklen_t peer_addrlen, xqc_cid_t *dcid, xqc_cid_t *scid,
                       xqc_conn_settings_t *settings, void *user_data)
{
    xqc_int_t ret;
    ...
    /*
     * Server enable cid negotiate, or client initial dcid length not equal to server
     * config length. If use the peer's dcid as scid directly, must make sure its length
     * equals to the config cid_len, otherwise might fail decoding dcid from subsequent
     * short header packets
     */
    if (engine->config->cid_negotiate || new_scid.cid_len != engine->config->cid_len) {
    ...
    xqc_gen_reset_token(&new_scid, new_scid.sr_token, XQC_STATELESS_RESET_TOKENLEN,
                        engine->config->reset_token_key,
                        engine->config->reset_token_keylen);

    conn = xqc_conn_create(engine, dcid, &new_scid, settings, user_data,
    ...
    if (xqc_cid_in_cid_set(&conn->scid_set, &conn->original_dcid, XQC_INITIAL_PATH_ID) ==
        NULL) {
        /*
         ...
         */
        if (xqc_insert_conns_hash(engine->conns_hash, conn, conn->original_dcid.cid_buf,
    ...
    ret = xqc_memcpy_with_cap(conn->local_addr, sizeof(conn->local_addr), local_addr,
                              local_addrlen);
    if (ret == XQC_OK) {
        conn->local_addrlen = local_addrlen;

    } else {
        xqc_log(conn->log, XQC_LOG_ERROR, "|local addr too large|addr_len:%d|",
                (int)local_addrlen);
        goto fail;
    }

    ret = xqc_memcpy_with_cap(conn->peer_addr, sizeof(conn->peer_addr), peer_addr,
                              peer_addrlen);
    if (ret == XQC_OK) {
        conn->peer_addrlen = peer_addrlen;

    } else {
        xqc_log(conn->log, XQC_LOG_ERROR, "|peer addr too large|addr_len:%d|",
                (int)peer_addrlen);
        goto fail;
    }

    ret = xqc_conn_server_init_path_addr(conn, XQC_INITIAL_PATH_ID, local_addr,
                                         local_addrlen, peer_addr, peer_addrlen);
    if (ret != XQC_OK) {
        goto fail;
    }
...
xqc_int_t
xqc_conn_client_on_alpn(xqc_connection_t *conn, const unsigned char *alpn,
                        size_t alpn_len)
{
    xqc_int_t ret;
    ...
    if (ret != XQC_OK) {
        ...
        conn->alpn = NULL;
        conn->alpn_len = 0;
        xqc_log(conn->log, XQC_LOG_ERROR, "|can't get application layer callback|ret:%d",
                ret);
        return ret;
    }
...
xqc_int_t
xqc_conn_server_on_alpn(xqc_connection_t *conn, const unsigned char *alpn,
                        size_t alpn_len)
{
    xqc_int_t ret;
    ...
    if (conn->app_proto_cbs.conn_cbs.conn_create_notify) {
        if (conn->app_proto_cbs.conn_cbs.conn_create_notify(
                conn, &conn->scid_set.user_scid, conn->user_data, conn->proto_data)) {
            goto err;
        }
    ...
    if (conn->conn_flag & XQC_CONN_FLAG_LOCAL_TP_UPDATED) {
        ret = xqc_conn_encode_local_tp(conn, tp_buf, XQC_MAX_TRANSPORT_PARAM_BUF_LEN,
                                       &tp_len);
        if (ret != XQC_OK) {
...
xqc_fec_conn_info_print(xqc_connection_t *conn, char *output, size_t output_size)
{
    ...
    ret = snprintf(
        ...
        conn->fec_ctl->fec_recover_pkt_cnt, conn->fec_ctl->fec_recover_failed_cnt,
        conn->fec_ctl->fec_flush_blk_cnt, conn->fec_ctl->fec_recv_repair_num_total,
        conn->fec_neg_fail_reason, conn->fec_ctl->fec_send_ahead,
        ...
        conn->fec_ctl->fec_enable_stream_num, conn->fec_ctl->conn_avg_recv_delay / 1000,
...
xqc_common_conn_info_print(xqc_connection_t *conn, char *output, size_t output_size)
{
    ...
    ret = snprintf(output, output_size, "%d,%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%d,",
                   conn->conn_calculated_frames, conn->conn_avg_close_delay / 1000,
                   conn->conn_avg_recv_delay / 1000, conn->conn_latest_close_delay / 1000,
                   conn->burst_loss_cnt);
...
xqc_conn_destroy(xqc_connection_t *xc)
{
    ...
    xqc_log(
        xc->log, XQC_LOG_REPORT,
        "|%p|"
        "has_0rtt:%d|0rtt_accept:%d|token_ok:%d|handshake_time:%ui|"
        "first_send_delay:%ui|conn_persist:%ui|keyupdate_cnt:%d|err:0x%xi|close_msg:%s|%"
        "s|"
        "hsk_recv:%ui|close_recv:%ui|close_send:%ui|last_recv:%ui|last_send:%ui|"
        "mp_enable:%ud|create:%ud|validated:%ud|active:%ud|path_info:%s|alpn:%*s|rebind_"
        "count:%d|"
        ...
        xc, xc->conn_flag & XQC_CONN_FLAG_HAS_0RTT ? 1 : 0,
        xc->conn_flag & XQC_CONN_FLAG_0RTT_OK ? 1 : 0,
        xc->conn_type == XQC_CONN_TYPE_SERVER
            ? (xc->conn_flag & XQC_CONN_FLAG_TOKEN_OK ? 1 : 0)
            : (-1),
        ...
        xc->enable_multipath, xc->create_path_count, xc->validated_path_count,
        xc->active_path_count, conn_stats.conn_info, out_alpn_len, out_alpn,
        conn_stats.total_rebind_count, conn_stats.total_rebind_valid,
        conn_stats.lost_count, conn_stats.tlp_count, conn_stats.send_count,
        conn_stats.spurious_loss_count, xc->detected_loss_cnt, xc->max_pto_cnt,
        xc->finished_streams, xc->cli_bidi_streams, xc->svr_bidi_streams, fec_flag,
        ensch_str, desch_str, xc->fec_neg_fail_reason, fec_mpm_str,
    ...
    if (xc->conn_settings.datagram_redundant_probe && xc->dgram_probe_timer >= 0) {
    ...
    if (xc->conn_type == XQC_CONN_TYPE_CLIENT) { /* client close connection */
        if ((xc->conn_flag & XQC_CONN_FLAG_UPPER_CONN_EXIST) &&
            ...
            xc->app_proto_cbs.conn_cbs.conn_close_notify(xc, &xc->scid_set.user_scid,
                                                         xc->user_data, xc->proto_data);
        } else {
            xqc_log(xc->log, XQC_LOG_REPORT, "|conn close event not notified|%s",
                    xqc_conn_addr_str(xc));
        }
    } else { /* server close connection */
        /* ALPN negotiated, notify close through application layer protocol callback
         * function */
        if ((xc->conn_flag & XQC_CONN_FLAG_UPPER_CONN_EXIST) &&
            xc->app_proto_cbs.conn_cbs.conn_close_notify) {
            xc->app_proto_cbs.conn_cbs.conn_close_notify(xc, &xc->scid_set.user_scid,
            ...
            xc->transport_cbs.server_refuse(xc->engine, xc, &xc->scid_set.user_scid,
                                            xc->user_data);
            xqc_log(xc->log, XQC_LOG_REPORT, "|conn close notified by refuse|%s",
                    xqc_conn_addr_str(xc));
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_paths_list)
    {
        ...
        if (path->path_state == XQC_PATH_STATE_ACTIVE) {
            ...
            ret = snprintf(buf + cursor, buf_sz - cursor,
                           ...
                           ",%" PRIu64 ",%" PRIu64 "),",
                           path->path_id, cwnd, inflight, sched_queue, est_bw,
                           pacing_rate, srtt_ms);
...
xqc_int_t
xqc_conn_send_ping_internal(xqc_connection_t *conn, void *ping_user_data,
                            xqc_bool_t notify)
{
    xqc_int_t ret;
    xqc_path_ctx_t *path;
    ...
    if (conn->enable_multipath && conn->conn_settings.mp_ping_on) {
        xqc_list_for_each_safe(pos, next, &conn->conn_paths_list)
        {
            ...
            if (path->path_state == XQC_PATH_STATE_ACTIVE) {
                ...
                if (ret < 0) {
                    xqc_log(conn->log, XQC_LOG_ERROR, "|write ping error|path:%ui|",
                            path->path_id);
...
xqc_conn_should_close(xqc_connection_t *conn, xqc_path_ctx_t *path)
{
    ...
    if (path->path_state < XQC_PATH_STATE_CLOSING) {
        ...
        if (ret != XQC_OK) {
            xqc_log(conn->log, XQC_LOG_ERROR, "|xqc_path_immediate_close error|ret:%d|",
                    ret);
        }
    }
...
xqc_send_burst(xqc_connection_t *conn, xqc_path_ctx_t *path, struct iovec *iov, int cnt)
{
    ...
    if (conn->pkt_filter_cb) {
        for (sent_cnt = 0; sent_cnt < cnt; sent_cnt++) {
            sent_size =
                conn->pkt_filter_cb(iov[sent_cnt].iov_base, iov[sent_cnt].iov_len,
            ...
            if (sent_size < 0) {
                xqc_log(conn->log, XQC_LOG_ERROR,
                        ...
                        conn, iov[sent_cnt].iov_len, sent_size, sent_cnt);
        ...
        sent_cnt = conn->transport_cbs.write_mmsg_ex(
            path->path_id, iov, cnt, (struct sockaddr *)path->peer_addr,
            path->peer_addrlen, xqc_conn_get_user_data(conn));
        ...
        sent_cnt = conn->transport_cbs.write_mmsg(
            iov, cnt, (struct sockaddr *)conn->peer_addr, conn->peer_addrlen,
...
xqc_int_t
xqc_check_acked_or_dropped_pkt(xqc_connection_t *conn, xqc_packet_out_t *packet_out,
                               xqc_send_type_t send_type)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG,
            "|conn:%p|pkt_num:%ui|size:%ud|pkt_type:%s|frame:%s|", conn,
                          ...
                          xqc_bool_t packets_are_limited_by_cc, xqc_send_type_t send_type)
{
    ...
    xqc_list_for_each_safe(pos, next, head)
    {
        ...
        if (xqc_packet_out_on_specific_path(conn, packet_out, &path)) {
            ...
            xqc_log(conn->log, XQC_LOG_DEBUG,
                    "|specify|path:%ui|state:%d|frame_type:%s|stream_id:%ui|stream_"
                    "offset:%ui|",
            ...
            path = conn->scheduler_callback->xqc_scheduler_get_path(
                conn->scheduler, conn, packet_out, packets_are_limited_by_cc, 0,
                &cc_blocked);
            if (path == NULL) {
                if (cc_blocked) {
    ...
    if (conn->conn_settings.fec_params.fec_encoder_scheme == XQC_PACKET_MASK_CODE &&
        ...
        if (cq_fin_timeout) {
            xqc_timer_set(&conn->conn_timer_manager, XQC_TIMER_QUEUE_FIN, now,
                          cq_fin_timeout * 1000);
        }
    }
}
...
static inline void
xqc_conn_log_sent_packet(xqc_connection_t *c, xqc_packet_out_t *po, xqc_usec_t timestamp)
{
    ...
    c->snd_pkt_stats.pkt_timestamp[index] =
        xqc_calc_delay(timestamp, c->conn_create_time);
...
void
xqc_on_packets_send_burst(xqc_connection_t *conn, xqc_path_ctx_t *path, ssize_t sent,
                          xqc_usec_t now, xqc_send_type_t send_type)
{
    xqc_list_head_t *pos, *next;
    xqc_packet_out_t *packet_out;
    ...
    xqc_list_for_each_safe(pos, next, &path->path_schedule_buf[send_type])
    {
        ...
        if (xqc_has_packet_number(&packet_out->po_pkt)) {
            ...
            if (XQC_CAN_IN_FLIGHT(packet_out->po_frame_types &&
                ...
                xqc_pacing_on_packet_sent(&send_ctl->ctl_pacing,
            ...
            if (XQC_IS_ACK_ELICITING(packet_out->po_frame_types)) {
                xqc_send_queue_insert_unacked(
                    packet_out,
                    &send_queue->sndq_unacked_packets[packet_out->po_pkt.pkt_pns],
                    send_queue);

            } else {
                xqc_send_queue_insert_free(packet_out, &send_queue->sndq_free_packets,
                                           send_queue);
            }
            xqc_log(conn->log, XQC_LOG_INFO,
                    "|<==|conn:%p|path:%ui|pkt_num:%ui|size:%ud|sent:%z|pkt_type:%s|"
                    "frame:%s|inflight:%ud|now:%ui|",
                    conn, path->path_id, packet_out->po_pkt.pkt_num,
                    packet_out->po_used_size, sent,
                    xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type),
                    xqc_frame_type_2_str(conn->engine, packet_out->po_frame_types),
                    send_ctl->ctl_bytes_in_flight, now);

        } else {
            /* packets with no packet number can't be acknowledged, hence they need no
             * control */
            xqc_path_send_buffer_remove(path, packet_out);
            xqc_send_queue_insert_free(packet_out, &send_queue->sndq_free_packets,
                                       send_queue);
...
xqc_convert_pkt_0rtt_2_1rtt(xqc_connection_t *conn, xqc_packet_out_t *packet_out)
{
    ...
    int ret =
        xqc_gen_short_packet_header(packet_out, conn->dcid_set.current_dcid.cid_buf,
                                    conn->dcid_set.current_dcid.cid_len, XQC_PKTNO_BITS,
                                    0, conn->key_update_ctx.cur_out_key_phase);
    packet_out->po_used_size = ret;

    if (ori_hdr_len < ret) {
        xqc_log(conn->log, XQC_LOG_ERROR,
                "|fatal|long_header_is_shorter_than_short_header|");
        XQC_CONN_ERR(conn, TRA_INTERNAL_ERROR);
        return;
    }
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG,
            "|0RTT to 1RTT|conn:%p|type:%d|pkt_num:%ui|pns:%d|frame:%s|", conn,
...
ssize_t
xqc_path_send_burst_packets(xqc_connection_t *conn, xqc_path_ctx_t *path, int congest,
                            xqc_send_type_t send_type)
{
    ssize_t ret;
    ...
    xqc_list_for_each_safe(pos, next, &path->path_schedule_buf[send_type])
    {
        ...
        if (xqc_has_packet_number(&packet_out->po_pkt)) {
            ...
            /* check the anti-amplification limit, will allow a bit larger than 3x
             * received */
            if (xqc_send_ctl_check_anti_amplification(send_ctl, total_bytes_to_send)) {
                xqc_log(conn->log, XQC_LOG_INFO,
                        "|blocked by anti amplification "
                        "limit|total_sent:%ui|3*total_recv:%ui|",
                        send_ctl->ctl_bytes_send + total_bytes_to_send,
                        3 * send_ctl->ctl_bytes_recv);
                break;
            }

            /* check cc limit */
            if (congest && !xqc_send_packet_check_cc(send_ctl, packet_out,
                                                     total_bytes_to_send, now)) {
            ...
            /* enc packet */
            ret = xqc_conn_enc_packet(
                conn, path, packet_out, iov_array[burst_cnt].iov_base,
            ...
            xqc_memcpy(iov_array[burst_cnt].iov_base, packet_out->po_buf,
                       packet_out->po_used_size);
    ...
    if (ret < 0) {
        ...
        xqc_log(conn->log, XQC_LOG_ERROR, "|error send msg|sent:%ui||cnt:%d|", ret,
                burst_cnt);
    }
...
void
xqc_path_send_packets(xqc_connection_t *conn, xqc_path_ctx_t *path, xqc_list_head_t *head,
                      int congest, xqc_send_type_t send_type)
{
    ...
    xqc_list_for_each_safe(pos, next, &path->path_schedule_buf[send_type])
    {
        ...
        if (xqc_check_acked_or_dropped_pkt(conn, packet_out, send_type)) {
            xqc_log(conn->log, XQC_LOG_DEBUG, "|path:%ui|canceled_bytes:%ud|reinj:%d|",
                    path->path_id, packet_out->po_used_size,
                    XQC_MP_PKT_REINJECTED(packet_out));
            continue;
        }
        ...
        /* check cc limit */
        if (congest && !xqc_send_packet_check_cc(send_ctl, packet_out, 0, now)) {
            send_ctl->ctl_conn->send_cc_blocked++;
            break;
        }
        ...
        if (XQC_IS_ACK_ELICITING(packet_out->po_frame_types)) {
            xqc_send_queue_insert_unacked(
                packet_out, &send_queue->sndq_unacked_packets[packet_out->po_pkt.pkt_pns],
                send_queue);
...
xqc_int_t
xqc_conn_enc_packet(xqc_connection_t *conn, xqc_path_ctx_t *path,
                    xqc_packet_out_t *packet_out, char *enc_pkt, size_t enc_pkt_cap,
                    size_t *enc_pkt_len, xqc_usec_t current_time)
{
    ...
    /* generate packet number and update packet length, might do packet number encoding
     * here */
    xqc_pn_ctl_t *pn_ctl = xqc_get_pn_ctl(conn, path);
    xqc_send_ctl_set_next_pn_for_packet(conn, pn_ctl, packet_out, current_time);

    xqc_write_packet_number(packet_out->po_ppktno, packet_out->po_pkt.pkt_num,
                            XQC_PKTNO_BITS);
    ...
    /* encrypt */
    xqc_int_t ret =
        xqc_packet_encrypt_buf(conn, packet_out, enc_pkt, enc_pkt_cap, enc_pkt_len);
    if (ret < 0) {
...
ssize_t
xqc_send(xqc_connection_t *conn, xqc_path_ctx_t *path, unsigned char *data,
         unsigned int len)
{
    ...
    if (conn->pkt_filter_cb) {
        ...
        if (sent < 0) {
            ...
            return sent == XQC_SOCKET_EAGAIN ? -XQC_EAGAIN
                                             : -XQC_EPACKET_FILETER_CALLBACK;
        }
        sent = len;
    ...
    if (conn->conn_type == XQC_CONN_TYPE_SERVER &&
        ...
        if (conn->transport_cbs.conn_send_packet_before_accept) {
            ...
            if (sent != len) {
                xqc_log(conn->log, XQC_LOG_ERROR,
                        "|send packet before accept error|conn:%p|size:%ud|sent:%z|",
                        conn, len, sent);
    ...
    if (conn->transport_cbs.write_socket_ex) {
        sent = conn->transport_cbs.write_socket_ex(
            path->path_id, data, len, (struct sockaddr *)path->peer_addr,
            path->peer_addrlen, xqc_conn_get_user_data(conn));
        if (sent != len) {
            xqc_log(conn->log, XQC_LOG_ERROR,
            ...
            if (sent == XQC_SOCKET_ERROR) {
                ...
                if (xqc_conn_should_close(conn, path)) {
                    xqc_log(conn->log, XQC_LOG_ERROR,
                            "|conn:%p|socket exception, close connection|", conn);
                    conn->conn_state = XQC_CONN_STATE_CLOSED;
                    xqc_log_event(conn->log, CON_CONNECTION_STATE_UPDATED, conn);
                }
            }
        ...
        sent = conn->transport_cbs.write_socket(
            data, len, (struct sockaddr *)conn->peer_addr, conn->peer_addrlen,
            xqc_conn_get_user_data(conn));
        if (sent != len) {
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|write_socket error|conn:%p|size:%ud|sent:%z|", conn, len, sent);
...
ssize_t
xqc_process_packet_without_pn(xqc_connection_t *conn, xqc_path_ctx_t *path,
                              xqc_packet_out_t *packet_out)
{
    /* directly send to peer */
    ssize_t sent = xqc_send(conn, path, packet_out->po_buf, packet_out->po_used_size);
    xqc_log(conn->log, XQC_LOG_INFO, "|<==|conn:%p|size:%ud|sent:%z|pkt_type:%s|", conn,
            packet_out->po_used_size, sent,
            xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type));
...
ssize_t
xqc_send_packet_with_pn(xqc_connection_t *conn, xqc_path_ctx_t *path,
                        xqc_packet_out_t *packet_out)
{
    ...
    if (sent != conn->enc_pkt_len) {
        xqc_log(conn->log, XQC_LOG_ERROR,
                "|write_socket "
                "error|conn:%p|path:%ui|pkt_num:%ui|size:%ud|sent:%z|pkt_type:%s|frame:%"
                "s|now:%ui|",
                conn, path->path_id, packet_out->po_pkt.pkt_num, packet_out->po_used_size,
                sent, xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type),
                xqc_frame_type_2_str(conn->engine, packet_out->po_frame_types), now);
        return sent;

    } else {
        xqc_log(conn->log, XQC_LOG_INFO,
                "|<==|conn:%p|path:%ui|pkt_num:%ui|size:%ud|sent:%z|pkt_type:%s|frame:%s|"
                "inflight:%ud|now:%ui|stream_id:%ui|stream_offset:%ui|",
                conn, path->path_id, packet_out->po_pkt.pkt_num, packet_out->po_used_size,
                sent, xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type),
                xqc_frame_type_2_str(conn->engine, packet_out->po_frame_types),
                path->path_send_ctl->ctl_bytes_in_flight, now, packet_out->po_stream_id,
                packet_out->po_stream_offset);
...
ssize_t
xqc_enc_packet_with_pn(xqc_connection_t *conn, xqc_path_ctx_t *path,
                       xqc_packet_out_t *packet_out)
{
    /* update dcid by send path */
    xqc_short_packet_update_dcid(packet_out, path->path_dcid);
    ...
    /*
     * 0RTT packets might be lost or retransmitted during handshake, once client get 1RTT
     * keys, it should retransmit the data with 1RTT packets instead.
     */
    if (XQC_UNLIKELY(packet_out->po_pkt.pkt_type == XQC_PTYPE_0RTT &&
                     conn->conn_flag & XQC_CONN_FLAG_CAN_SEND_1RTT)) {
        xqc_convert_pkt_0rtt_2_1rtt(conn, packet_out);
...
ssize_t
xqc_process_packet_with_pn(xqc_connection_t *conn, xqc_path_ctx_t *path,
                           xqc_packet_out_t *packet_out)
{
    ssize_t ret = xqc_enc_packet_with_pn(conn, path, packet_out);
    if (ret != XQC_OK) {
        xqc_log(conn->log, XQC_LOG_ERROR,
                "|path:%ui|pkt_type:%d|pn:%ui|frames:%ui|size:%ud|", path->path_id,
                packet_out->po_pkt.pkt_type, packet_out->po_pkt.pkt_num,
                (uint64_t)packet_out->po_frame_types, packet_out->po_used_size);
        return ret;
    }
...
ssize_t
xqc_path_send_one_packet(xqc_connection_t *conn, xqc_path_ctx_t *path,
                         xqc_packet_out_t *packet_out)
{
    if (xqc_has_packet_number(&packet_out->po_pkt)) {
...
xqc_conn_check_path_utilization(xqc_connection_t *conn)
{
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_paths_list)
    {
        ...
        if (!xqc_path_is_full(path) &&
            ...
            conn->scheduler_callback->xqc_scheduler_handle_path_event(
                conn->scheduler, path, XQC_SCHED_EVENT_PATH_NOT_FULL, NULL);
        }
    }
}
...
xqc_conn_schedule_start(xqc_connection_t *conn)
{
    ...
    if (conn->scheduler_callback->xqc_scheduler_handle_conn_event) {
        conn->scheduler_callback->xqc_scheduler_handle_conn_event(
            conn->scheduler, conn, XQC_SCHED_EVENT_CONN_ROUND_START, NULL);
    }
}
...
xqc_conn_schedule_end(xqc_connection_t *conn)
{
    ...
    if (conn->scheduler_callback->xqc_scheduler_handle_conn_event) {
        conn->scheduler_callback->xqc_scheduler_handle_conn_event(
            conn->scheduler, conn, XQC_SCHED_EVENT_CONN_ROUND_FIN, NULL);
    }
}
...
xqc_conn_gen_ping(xqc_connection_t *conn, xqc_pkt_num_space_t pns)
{
    ...
    switch (pns) {
    case XQC_PNS_INIT: ptype = XQC_PTYPE_INIT; break;

    case XQC_PNS_HSK: ptype = XQC_PTYPE_HSK; break;

    case XQC_PNS_APP_DATA: ptype = XQC_PTYPE_SHORT_HEADER; break;
                            ...
                            xqc_path_specified_flag_t flag)
{
    ...
    /* put PING into probe list, which is not limited by amplification or
     * congestion-control */
    xqc_send_queue_remove_send(&packet_out->po_list);
    xqc_send_queue_insert_probe(&packet_out->po_list,
                                &conn->conn_send_queue->sndq_pto_probe_packets);
                        ...
                        xqc_packet_out_t *packet_out)
{
    ...
    xqc_log(c->log, XQC_LOG_DEBUG,
            ...
            "|size:%ud|pkt_type:%s|frame:%s|conn_state:%s|",
            c, packet_out->po_path_id, packet_out->po_pkt.pkt_num,
            packet_out->po_used_size, xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type),
            xqc_frame_type_2_str(c->engine, packet_out->po_frame_types),
            xqc_conn_state_2_str(c->conn_state));

    /* reinjection */
    if (c->enable_multipath && c->reinj_callback &&
        c->reinj_callback->xqc_reinj_ctl_can_reinject &&
        c->reinj_callback->xqc_reinj_ctl_can_reinject(c->reinj_ctl, packet_out, mode)) {
        if (xqc_conn_try_reinject_packet(c, packet_out) == XQC_OK) {
            xqc_log(c->log, XQC_LOG_DEBUG,
                    "|MP|REINJ|reinject pto packets|"
...
xqc_path_send_one_or_two_ack_elicit_pkts(xqc_path_ctx_t *path, xqc_pkt_num_space_t pns)
{
    ...
    xqc_list_for_each_safe(pos, next, sndq)
    {
        ...
        if (XQC_IS_ACK_ELICITING(packet_out->po_frame_types) &&
            ...
            has_reinjection =
                has_reinjection || xqc_conn_send_probe_pkt(c, path, packet_out);
            ...
            if (send_hsd && (packet_out->po_frame_types & XQC_FRAME_BIT_HANDSHAKE_DONE)) {
                send_hsd = XQC_FALSE;
    ...
    if (probe_num > 0) {
        if (packet_out_last_sent) {
            ...
            while (probe_num > 0) {
                ...
                has_reinjection = has_reinjection ||
...
xqc_conn_close(xqc_engine_t *engine, const xqc_cid_t *cid)
{
    ...
    if (conn->conn_settings.linger.linger_on &&
        !xqc_send_queue_out_queue_empty(conn->conn_send_queue)) {
...
xqc_conn_immediate_close(xqc_connection_t *conn)
{
    ...
    if (!(conn->conn_flag & XQC_CONN_FLAG_INIT_RECVD) &&
        ...
        conn->conn_state = XQC_CONN_STATE_CLOSED;
        xqc_log_event(conn->log, CON_CONNECTION_STATE_UPDATED, conn);
        xqc_conn_log(conn, XQC_LOG_ERROR,
                     "|server cannot send CONNECTION_CLOSE before initial pkt received|");
        return XQC_OK;
    }
    ...
    /*
     * [Transport] 10.3.  Immediate Close, During the closing period, an endpoint that
     * sends a CONNECTION_CLOSE frame SHOULD respond to any incoming packet that can be
     * decrypted with another packet containing a CONNECTION_CLOSE frame.  Such an
    ...
    if (conn->conn_close_count < MAX_RSP_CONN_CLOSE_CNT) {
        ...
        if (ret) {
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|xqc_write_conn_close_to_packet error|ret:%d|", ret);
        }
        ++conn->conn_close_count;
        xqc_log(conn->log, XQC_LOG_DEBUG, "|gen_conn_close|state:%s|",
                xqc_conn_state_2_str(conn->conn_state));
    }
...
xqc_conn_send_retry(xqc_connection_t *conn, unsigned char *token, unsigned token_len)
{
    ...
    xqc_int_t size = (xqc_int_t)xqc_gen_retry_packet(
        conn, buf, conn->dcid_set.current_dcid.cid_buf,
        conn->dcid_set.current_dcid.cid_len, conn->retry_scid.cid_buf,
        conn->retry_scid.cid_len, conn->original_dcid.cid_buf,
        conn->original_dcid.cid_len, token, token_len, XQC_VERSION_V1);
    if (size < 0) {
        xqc_log(engine->log, XQC_LOG_WARN,
                "|xqc_conn_send_retry|generate retry packet failed|"
                "size:%d|",
                size);
        return XQC_ERROR;
    }
    ...
    if (send != size) {
        xqc_log(engine->log, XQC_LOG_WARN,
                "|xqc_conn_send_retry|send retry packet failed|"
                "size:%d|send:%d|",
                size, send);
        return XQC_ERROR;
    }
    ...
    if (xqc_insert_conns_hash(conn->engine->conns_hash, conn, conn->retry_scid.cid_buf,
...
xqc_conn_send_version_negotiation(xqc_connection_t *c)
{
    xqc_packet_out_t *packet_out = xqc_packet_out_get_and_insert_send(
        c->conn_send_queue, XQC_PTYPE_VERSION_NEGOTIATION);
    if (packet_out == NULL) {
...
xqc_conn_info_print(xqc_connection_t *conn, xqc_conn_stats_t *conn_stats)
{
    ...
    init_cwnd = conn->conn_settings.cc_params.customize_on
                    ? conn->conn_settings.cc_params.init_cwnd
                    : 0;
    ...
    ret = snprintf(buff, buff_size,
                   ...
                   mp_settings, conn->create_path_count, conn->validated_path_count,
                   conn->active_path_count, conn->dgram_stats.total_dgram,
                   conn->dgram_stats.hp_dgram, conn->dgram_stats.hp_red_dgram,
                   conn->dgram_stats.hp_red_dgram_mp, conn->dgram_stats.timer_red_dgram,
                   conn->sched_cc_blocked, conn->send_cc_blocked,
                   conn->snd_pkt_stats.conn_sent_pkts, conn->rcv_pkt_stats.conn_rcvd_pkts,
                   conn->rcv_pkt_stats.conn_udp_pkts, conn->stream_stats.send_bytes,
                   conn->stream_stats.reinjected_bytes, conn->stream_stats.recv_bytes,
                   init_cwnd);
    ...
    for (i = 0; i < 3; i++) {
        ret =
            snprintf(buff + curr_size, buff_size - curr_size,
                     "%u,%u,%u,%" PRIx64 ",%" PRIu64 ","
                     "%" PRIu64 ",%d,",
                     (uint32_t)conn->rcv_pkt_stats.pkt_types[i],
                     conn->rcv_pkt_stats.pkt_size[i], conn->rcv_pkt_stats.pkt_udp_size[i],
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_paths_list)
    {
        ...
        ret = snprintf(
            buff + curr_size, buff_size - curr_size,
            ...
            "%d-%u,%d-%u,"
            "%d-%u,%d-%u,",
            (int)path_info.path_id, path_info.path_state, (int)path_info.path_id,
            path_info.app_path_status, (int)path_info.path_id, sock_err_flag,
            (int)path_info.path_id, path_info.path_create_time, (int)path_info.path_id,
            path_info.path_destroy_time, (int)path_info.path_id, path_info.srtt / 1000,
            (int)path_info.path_id, path_info.path_bytes_send, (int)path_info.path_id,
            path_info.path_bytes_recv, (int)path_info.path_id, path_info.pkt_send_cnt,
            (int)path_info.path_id, path_info.pkt_recv_cnt, (int)path_info.path_id,
            path_info.loss_cnt, (int)path_info.path_id, path_info.tlp_cnt,
            (int)path_info.path_id, path_info.dgram_send_cnt, (int)path_info.path_id,
            path_info.dgram_recv_cnt, (int)path_info.path_id,
            path_info.red_dgram_send_cnt, (int)path_info.path_id,
            path_info.red_dgram_recv_cnt, (int)path_info.path_id, path->rebinding_count,
...
xqc_conn_get_stats_internal(xqc_connection_t *conn, xqc_conn_stats_t *conn_stats)
{
    ...
    if (conn->conn_initial_path && conn->conn_initial_path->path_send_ctl &&
        ...
        xqc_recv_record_print(
            conn,
            &conn->conn_initial_path->path_pn_ctl->ctl_recv_record[XQC_PNS_APP_DATA],
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_paths_list)
    {
        ...
        send_ctl = path->path_send_ctl;
        if (send_ctl == NULL) {
            xqc_log(conn->log, XQC_LOG_WARN, "|path_send_ctl NULL|path%ui|",
                    path->path_id);
            continue;
        }
...
xqc_int_t
xqc_conn_decrypt_token(xqc_connection_t *conn, unsigned char *dec_data,
                       unsigned int *d_len, const unsigned char *enc_data, unsigned e_len)
{
    ...
    *d_len += tmp_len;
    if (XQC_SSL_SUCCESS != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG,
                                               XQC_TOKEN_TAG_LEN,
                                               (void *)(p_enc + len))) {
...
xqc_int_t
xqc_conn_check_token(xqc_connection_t *conn, const unsigned char *enc_data,
                     unsigned e_len)
{
    ...
    if (*pos++ & 0x80) {
        ...
        if (token_len != 21) {
            xqc_log(conn->log, XQC_LOG_ERROR, "|token_len error|token_len:%ui|",
                    token_len);
            return XQC_ERROR;
        }
        ...
        if (memcmp(&sa4->sin_addr, pos, sizeof(struct in_addr)) != 0) {
            xqc_log(conn->log, XQC_LOG_INFO, "|ipv4 not match|token_addr:%s|",
                    inet_ntoa(*in4));
            return XQC_ERROR;
        }
    ...
    *expire = ntohl(*expire);
    ...
    if (*expire < now) {
        xqc_log(conn->log, XQC_LOG_INFO, "|token_expire|expire:%ud|now:%ui|", *expire,
                now);
        return XQC_ERROR;
...
void
xqc_conn_generate_plain_token(xqc_connection_t *conn, uint8_t ts_index,
                              unsigned char *token, unsigned *token_len, uint32_t expire)
{
    struct sockaddr *sa = (struct sockaddr *)conn->peer_addr;
                                    ...
                                    unsigned *token_len, uint32_t expire)
{
    ...
    *token_len += len;
    if (XQC_SSL_SUCCESS != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG,
                                               XQC_TOKEN_TAG_LEN, token + *token_len)) {
        goto error;
    }
...
xqc_int_t
xqc_conn_gen_retry_token(xqc_connection_t *conn, unsigned char *token,
                         unsigned *token_len)
{
    uint32_t expire =
        xqc_monotonic_timestamp() / 1000000 + XQC_TOKEN_RETRY_PACKET_EXPIRE_DELTA;
    expire = htonl(expire);
    return xqc_conn_generate_and_encrypt_token(conn, token, token_len, expire);
}
...
xqc_conn_resend_0rtt_datagram(xqc_connection_t *conn)
{
    ...
    xqc_list_for_each_safe(pos, next, &conn->dgram_0rtt_buffer_list)
    {
        ...
        if (iov_size >= XQC_MAX_SEND_MSG_ONCE) {
            ret = xqc_datagram_send_multiple_internal(
                conn, iov, dgram_id_list, XQC_MAX_SEND_MSG_ONCE, &sent, &sent_bytes,
                dgram_buffer->qos_level, XQC_TRUE);
            if (ret < 0) {
                xqc_log(conn->log, XQC_LOG_ERROR,
    ...
    if (iov_size > 0) {
        ret = xqc_datagram_send_multiple_internal(conn, iov, dgram_id_list, iov_size,
                                                  &sent, &sent_bytes,
                                                  dgram_buffer->qos_level, XQC_TRUE);
        if (ret < 0) {
...
xqc_conn_server_accept(xqc_connection_t *c)
{
    ...
    if (!(c->conn_flag & XQC_CONN_FLAG_TOKEN_OK)) {
        ...
        if (xqc_conn_check_token(c, c->conn_token, c->conn_token_len) == XQC_OK) {
        ...
        } else if (c->version != XQC_IDRAFT_VER_29) { /* IDRAFT_VER_29 is too old to send
                                                         retry packet */
            ...
            if (c->conn_flag & XQC_CONN_FLAG_RETRY_SENT) {
                ...
                if (c->transport_cbs.conn_retry_packet_condition_check) {
                    if (c->transport_cbs.conn_retry_packet_condition_check(
                            c->engine, c, &c->scid_set.user_scid, c->user_data) ==
                        XQC_TRUE) {
                        ret = xqc_conn_send_retry_packet(c);
                        if (ret == XQC_OK) {
                            return -XQC_EIGNORE_PKT; /* send retry packet, then ignore
                                                        this packet */
                        } else {
    ...
    if (c->transport_cbs.server_accept) {
        ret = c->transport_cbs.server_accept(c->engine, c, &c->scid_set.user_scid,
                                             c->user_data);
        if (ret != XQC_OK) {
...
xqc_conn_try_to_enable_multipath(xqc_connection_t *conn)
{
    ...
    return xqc_conn_add_path_cid_sets(conn, XQC_INITIAL_PATH_ID + 1,
                                      conn->curr_max_path_id);
}
...
xqc_conn_handshake_complete(xqc_connection_t *conn)
{
    ...
    if (conn->enable_multipath) {
        ...
        if (mp_aead_ret == -(xqc_int_t)TRA_TRANSPORT_PARAMETER_ERROR) {
            ...
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|mp21|mp aead nonce-len check internal error|ret:%d|", mp_aead_ret);
            XQC_CONN_ERR(conn, TRA_INTERNAL_ERROR);
            return mp_aead_ret;
        }
    }
    ...
    if (conn->conn_type == XQC_CONN_TYPE_SERVER) {
        /* the TLS handshake is considered confirmed at the server when the handshake
         * completes */
        ...
        /* if client sent no token or sent an invalid token, server sends a NEW_TOKEN
         * frame */
...
xqc_int_t
xqc_conn_buff_undecrypt_packet_in(xqc_packet_in_t *packet_in, xqc_connection_t *conn,
                                  xqc_encrypt_level_t encrypt_level)
{
    ...
    /* limit the buffered 0-RTT packet count before a valid client Initial packet is
     * received */
    if (conn->conn_type == XQC_CONN_TYPE_SERVER &&
        !(conn->conn_flag & XQC_CONN_FLAG_DCID_OK) && encrypt_level == XQC_ENC_LEV_0RTT &&
    ...
    new_packet->pos =
        (unsigned char *)new_packet->buf + (packet_in->pos - packet_in->buf);
    new_packet->last =
        (unsigned char *)new_packet->buf + (packet_in->last - packet_in->buf);
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG,
            "|====>|delay|undecrypt_count:%ud|encrypt_level:%d|",
...
xqc_int_t
xqc_conn_process_undecrypt_packet_in(xqc_connection_t *conn,
                                     xqc_encrypt_level_t encrypt_level)
{
    ...
    {
        packet_in = xqc_list_entry(pos, xqc_packet_in_t, pi_list);
        xqc_log_event(conn->log, TRA_DATAGRAMS_RECEIVED, packet_in->buf_size,
                      packet_in->pi_path_id);
        xqc_log(conn->log, XQC_LOG_DEBUG, "|delay|undecrypt_count:%ud|encrypt_level:%d|",
                conn->undecrypt_count[encrypt_level], encrypt_level);
        ret = xqc_conn_process_packet(conn, packet_in->buf, packet_in->buf_size,
...
xqc_conn_buff_1rtt_packet(xqc_connection_t *conn, xqc_packet_out_t *po)
{
    ...
    xqc_send_queue_insert_buff(&po->po_list,
                               &conn->conn_send_queue->sndq_buff_1rtt_packets);
...
xqc_conn_buff_1rtt_packets(xqc_connection_t *conn)
{
    ...
    xqc_list_for_each_safe(pos, next, &conn->conn_send_queue->sndq_send_packets)
    {
        ...
        if (packet_out->po_pkt.pkt_type == XQC_PTYPE_SHORT_HEADER) {
            xqc_send_queue_remove_send(&packet_out->po_list);
            xqc_send_queue_insert_buff(&packet_out->po_list,
                                       &conn->conn_send_queue->sndq_buff_1rtt_packets);
            if (!(conn->conn_flag & XQC_CONN_FLAG_DCID_OK)) {
...
xqc_conn_write_buffed_1rtt_packets(xqc_connection_t *conn)
{
    if (conn->conn_flag & XQC_CONN_FLAG_CAN_SEND_1RTT) {
        ...
        xqc_list_for_each_safe(pos, next, &send_queue->sndq_buff_1rtt_packets)
        {
            ...
            xqc_send_queue_insert_send(packet_out, &send_queue->sndq_send_packets,
                                       send_queue);
...
char *
xqc_local_addr_str(xqc_engine_t *engine, const struct sockaddr *local_addr,
                   socklen_t local_addrlen)
{
    ...
    if (sa_local->sin_family == AF_INET) {
        if (inet_ntop(sa_local->sin_family, &sa_local->sin_addr, engine->local_addr_str,
                      local_addrlen) == NULL) {
            engine->local_addr_str[0] = '\0';
        }
...
char *
xqc_peer_addr_str(xqc_engine_t *engine, const struct sockaddr *peer_addr,
                  socklen_t peer_addrlen)
{
    ...
    if (sa_peer->sin_family == AF_INET) {
        if (inet_ntop(sa_peer->sin_family, &sa_peer->sin_addr, engine->peer_addr_str,
                      peer_addrlen) == NULL) {
            engine->peer_addr_str[0] = '\0';
        }
...
xqc_conn_addr_str(xqc_connection_t *conn)
{
    if (conn->local_addrlen == 0 || conn->peer_addrlen == 0 ||
        ...
        conn->dcid_set.current_dcid.cid_len == 0) {
    ...
    if (conn->addr_str_len == 0) {
        ...
        conn->addr_str_len =
            snprintf(conn->addr_str, sizeof(conn->addr_str), "l-%s-%d-%s p-%s-%d-%s",
                     xqc_local_addr_str(conn->engine, (struct sockaddr *)sa_local,
                                        conn->local_addrlen),
                     ntohs(sa_local->sin_port),
                     xqc_scid_str(conn->engine, &conn->scid_set.user_scid),
                     xqc_peer_addr_str(conn->engine, (struct sockaddr *)sa_peer,
                                       conn->peer_addrlen),
...
xqc_path_addr_str(xqc_path_ctx_t *path)
{
    ...
    if (path->addr_str_len == 0) {
        ...
        path->addr_str_len =
            snprintf(path->addr_str, sizeof(path->addr_str), "l-%s-%d-%s p-%s-%d-%s",
                     xqc_local_addr_str(path->parent_conn->engine,
                                        (struct sockaddr *)sa_local, path->local_addrlen),
                     ntohs(sa_local->sin_port),
                     xqc_scid_str(path->parent_conn->engine, &path->path_scid),
                     xqc_peer_addr_str(path->parent_conn->engine,
                                       (struct sockaddr *)sa_peer, path->peer_addrlen),
                     ntohs(sa_peer->sin_port),
                     xqc_dcid_str(path->parent_conn->engine, &path->path_dcid));
    }
...
xqc_conn_record_single(xqc_connection_t *c, xqc_packet_in_t *packet_in)
{
    ...
    if ((c->conn_settings.extended_ack_features & XQC_ACK_EXT_FEATURE_BIT_RECV_TS) &&
        ...
        xqc_recv_timestamps_info_add_pkt(path->recv_ts_info, pkt_num,
                                         packet_in->pkt_recv_time);
    }
    ...
    if (range_status == XQC_PKTRANGE_OK) {
        if (XQC_IS_ACK_ELICITING(packet_in->pi_frame_types)) {
            ...
            if (pkt_num > send_ctl->ctl_largest_received[pns] ||
                send_ctl->ctl_largest_received[pns] == XQC_MAX_UINT64_VALUE) {
        ...
        xqc_maybe_should_ack(c, path, pn_ctl, pns, out_of_order,
                             packet_in->pkt_recv_time);
    }

    // xqc_recv_record_log(c, &pn_ctl->ctl_recv_record[pns]);
    xqc_log(c->log, XQC_LOG_DEBUG,
            "|path:%ui|xqc_recv_record_add|status:%d|pkt_num:%ui|largest:%ui|pns:%d|",
            path->path_id, range_status, pkt_num,
            xqc_recv_record_largest(&pn_ctl->ctl_recv_record[pns]), pns);
}
...
xqc_conn_confirm_cid(xqc_connection_t *c, xqc_packet_t *pkt)
{
    ...
    if (!(c->conn_flag & XQC_CONN_FLAG_DCID_OK)) {
        if (xqc_cid_in_cid_set(&c->dcid_set, &pkt->pkt_scid, XQC_INITIAL_PATH_ID) ==
            NULL) {
        ...
        if (XQC_OK != xqc_cid_is_equal(&c->dcid_set.current_dcid, &pkt->pkt_scid)) {
            xqc_log(c->log, XQC_LOG_INFO, "|dcid change|ori:%s|new:%s|",
                    xqc_dcid_str(c->engine, &c->dcid_set.current_dcid),
                    xqc_scid_str(c->engine, &pkt->pkt_scid));
...
xqc_conn_server_validate_address(xqc_connection_t *c, xqc_packet_in_t *pi)
{
    ...
    case XQC_PTYPE_INIT:
        if (XQC_CONN_FLAG_TOKEN_OK & c->conn_flag) {
            ...
            /**
             * when server close its own CID, and server reached its anti-amplification
             * limit, client MAY send an Initial packet with PING/PADDING on PTO with
             * server's CID
             */
            if (c->scid_set.user_scid.cid_len >= XQC_CONN_ADDR_VALIDATION_CID_ENTROPY &&
                xqc_cid_in_cid_set(&c->scid_set, &c->original_dcid,
                                   XQC_INITIAL_PATH_ID) == NULL &&
                xqc_cid_in_cid_set(&c->scid_set, &pi->pi_pkt.pkt_dcid,
                                   XQC_INITIAL_PATH_ID) != NULL) {
                xqc_conn_addr_validated(c);
            }
        }
    ...
    /*
     * loss detection timer might be unset when anti-amplification limit is reached, but
     * receiving a handshake or receiving a packet with cid which was choosed by server,
     * will remove the anti-amplification state, and loss detection timer shall be
     * re-armed.
     */
...
xqc_conn_client_validate_address(xqc_connection_t *c, xqc_packet_in_t *pi)
{
    ...
    case XQC_PTYPE_VERSION_NEGOTIATION: xqc_conn_addr_validated(c); break;
...
xqc_conn_on_hsk_processed(xqc_connection_t *c, xqc_packet_in_t *pi, xqc_usec_t now)
{
    ...
    if (c->conn_type == XQC_CONN_TYPE_SERVER) {
        /* server MUST discard Initial keys when it first successfully processes a
         * Handshake packet */
...
xqc_conn_on_pkt_processed(xqc_connection_t *c, xqc_packet_in_t *pi, xqc_usec_t now)
{
    xqc_int_t ret = XQC_OK;
    switch (pi->pi_pkt.pkt_type) {
    case XQC_PTYPE_INIT: ret = xqc_conn_on_initial_processed(c, pi, now); break;

    case XQC_PTYPE_HSK: ret = xqc_conn_on_hsk_processed(c, pi, now); break;

    case XQC_PTYPE_SHORT_HEADER: ret = xqc_conn_on_1rtt_processed(c, pi, now); break;
    ...
    if (pi->pi_frame_types &
        (~(XQC_FRAME_BIT_STREAM | XQC_FRAME_BIT_DATAGRAM | XQC_FRAME_BIT_PADDING |
    ...
    xqc_log(c->log, XQC_LOG_INFO,
            "|====>|conn:%p|path:%ui|size:%uz|pkt_type:%s|pkt_num:%ui|frame:%s|recv_time:"
            "%ui|dcid:%s|dcid_seq:%ui|",
            c, pi->pi_path_id, pi->buf_size, xqc_pkt_type_2_str(pi->pi_pkt.pkt_type),
            pi->pi_pkt.pkt_num, xqc_frame_type_2_str(c->engine, pi->pi_frame_types),
            pi->pkt_recv_time, xqc_dcid_str(c->engine, &pi->pi_pkt.pkt_dcid),
...
static inline void
xqc_conn_log_recvd_packet(xqc_connection_t *c, xqc_packet_in_t *pi, size_t udp_size,
                          xqc_int_t err, xqc_usec_t timestamp)
{
    ...
    c->rcv_pkt_stats.pkt_timestamp[index] =
        xqc_calc_delay(timestamp, c->conn_create_time);
                        ...
                        size_t packet_in_size, xqc_usec_t recv_time)
{
    ...
    while (pos < end) {
        ...
        if (ret != XQC_OK || last_pos == packet_in->pos) {
            /* if last_pos equals packet_in->pos, might trigger infinite loop, return to
             * avoid it */
            xqc_log(c->log, XQC_LOG_ERROR,
                    "|process packets err|ret:%d|pos:%p|buf:%p|buf_size:%uz|", ret,
                    packet_in->pos, packet_in->buf, packet_in->buf_size);
                                    ...
                                    size_t packet_in_size, xqc_usec_t recv_time)
{
    ...
    if (path == NULL) {
        xqc_log(conn->log, XQC_LOG_INFO, "|ignore unknown path|scid:%s|",
                xqc_scid_str(conn->engine, scid));
        return;
    }
...
xqc_conn_check_handshake_complete(xqc_connection_t *conn)
{
    ...
    if (!(conn->conn_flag & XQC_CONN_FLAG_HANDSHAKE_COMPLETED) &&
        ...
        if (conn->app_proto_cbs.conn_cbs.conn_handshake_finished) {
            conn->app_proto_cbs.conn_cbs.conn_handshake_finished(conn, conn->user_data,
                                                                 conn->proto_data);
        }
    }
...
xqc_conn_get_available_path_id(xqc_connection_t *conn, uint64_t *path_id)
{
    if (conn->enable_multipath &&
        ...
        /* principle: the next unused path ID has at least one unused DCID and one acked
         * unused SCID */
        xqc_cid_set_inner_t *scid_inner_set =
            xqc_get_next_unused_path_cid_set(&conn->scid_set);
        if (scid_inner_set) {
            xqc_cid_set_inner_t *dcid_inner_set =
                xqc_get_path_cid_set(&conn->dcid_set, scid_inner_set->path_id);
            if (dcid_inner_set) {
    ...
    if (conn->engine->conns_hash) {
        ...
        xqc_list_for_each_safe(pos_set, next_set, &conn->scid_set.cid_set_list)
        {
            ...
            xqc_list_for_each_safe(pos, next, &inner_set->cid_list)
            {
                ...
                if (xqc_find_conns_hash(conn->engine->conns_hash, conn, cid->cid.cid_buf,
                                        cid->cid.cid_len)) {
                    xqc_remove_conns_hash(conn->engine->conns_hash, conn,
    ...
    if (xqc_conn_is_handshake_confirmed(conn)) {
        if (conn->enable_multipath &&
            ...
            xqc_list_for_each_safe(pos, next, &conn->scid_set.cid_set_list)
            {
                ...
                if (inner_set->set_state == XQC_CID_SET_USED) {
                           ...
                           && inner_set->unused_cnt < unused_limit) 
                    {
                        ...
                        if (ret != XQC_OK) {
                            xqc_log(conn->log, XQC_LOG_ERROR,
                                    "|xqc_write_mp_new_conn_id_frame_to_packet "
                                    "error|path_id:%ui|",
                                    inner_set->path_id);
                            return ret;
                        }
                   ...
                   && inner_set->unused_cnt < unused_limit)
            {
                ...
                if (ret != XQC_OK) {
                    xqc_log(conn->log, XQC_LOG_ERROR,
                            "|xqc_write_new_conn_id_frame_to_packet error|");
                    return ret;
                }
            }
        }
...
xqc_conn_confirm_key_update(xqc_connection_t *conn)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG, "|key phase changed to %ui|",
            ctx->cur_out_key_phase);
     ...
     * decrypt packets that initiate a key update during the period when the peer retains
     * old keys. Endpoints SHOULD wait three times the PTO before initiating a key update
     * after receiving an acknowledgment that confirms that the previous key update was
     * received. Failing to allow sufficient time could lead to packets being discarded.
     */
...
xqc_conn_check_dcid(xqc_connection_t *conn, xqc_cid_t *dcid)
{
    ...
    if (scid->state == XQC_CID_UNUSED) {
        ret = xqc_cid_switch_to_next_state(&conn->scid_set, scid, XQC_CID_USED,
                                           scid->cid.path_id);
        if (ret < 0) {
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|xqc_cid_switch_to_next_state error|scid:%s|",
...
xqc_conn_set_cid_retired_ts(xqc_connection_t *conn, xqc_cid_inner_t *inner_cid)
{
    ...
    ret = xqc_cid_switch_to_next_state(&conn->scid_set, inner_cid, XQC_CID_RETIRED,
                                       inner_cid->cid.path_id);
    if (ret != XQC_OK) {
        xqc_log(conn->log, XQC_LOG_ERROR, "|set cid retired error|");
        return ret;
    }

    xqc_log(conn->log, XQC_LOG_DEBUG, "|retired|cid:%s|seq:%ui|len:%d|",
            xqc_scid_str(conn->engine, &inner_cid->cid), inner_cid->cid.cid_seq_num,
        ...
        xqc_list_for_each_safe(pos, next, &inner_set->cid_list)
        {
            ...
            if (scid->state == XQC_CID_USED &&
                ...
                if (conn->transport_cbs.conn_update_cid_notify) {
                    conn->transport_cbs.conn_update_cid_notify(
                        conn, &conn->scid_set.user_scid, &scid->cid,
                        xqc_conn_get_user_data(conn));
                }
...
xqc_conn_peer_complete_address_validation(xqc_connection_t *c)
{
    ...
    if (c->conn_type == XQC_CONN_TYPE_SERVER) {
        ...
        return (c->conn_flag & XQC_CONN_FLAG_HANDSHAKE_CONFIRMED) ||
               xqc_send_ctl_ack_received_in_pns(c->conn_initial_path->path_send_ctl,
                                                XQC_PNS_HSK);
    }
}
                                     ...
                                     xqc_transport_params_type_t exttype)
{
    ...
    if (params->stateless_reset_token_present) {
        ...
        xqc_memset(settings->stateless_reset_token, 0,
                   sizeof(settings->stateless_reset_token));
    }
                                    ...
                                    xqc_transport_params_t *params)
{
    ...
    if (conn->conn_type == XQC_CONN_TYPE_SERVER && conn->original_dcid.cid_len > 0) {
        xqc_cid_set(&params->original_dest_connection_id, conn->original_dcid.cid_buf,
                    conn->original_dcid.cid_len);
        params->original_dest_connection_id_present = 1;

        xqc_gen_reset_token(&conn->original_dcid, params->stateless_reset_token,
                            XQC_STATELESS_RESET_TOKENLEN,
    ...
    xqc_cid_set(&params->initial_source_connection_id, conn->initial_scid.cid_buf,
                conn->initial_scid.cid_len);
    params->initial_source_connection_id_present = 1;

    if (conn->conn_type == XQC_CONN_TYPE_SERVER &&
        conn->conn_flag & XQC_CONN_FLAG_RETRY_SENT && conn->retry_scid.cid_len > 0) {
        xqc_cid_set(&params->retry_source_connection_id, conn->retry_scid.cid_buf,
                    conn->retry_scid.cid_len);
...
xqc_conn_check_transport_params(xqc_connection_t *conn, const xqc_transport_params_t *params)
{
    ...
    if (conn->conn_type == XQC_CONN_TYPE_SERVER) {
        ...
        /* RFC 9000 §7.3: validate initial_source_connection_id matches peer's first
         * Initial SCID */
...
xqc_check_fec_trans_param(xqc_connection_t *conn, xqc_transport_params_t params)
{
    if (params.enable_encode_fec) {
        if (params.fec_max_symbols_num > XQC_FEC_MAX_SYMBOL_NUM_PBLOCK) {
            ...
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|quic_fec|remote_setting's max_symbol_number is too large to be "
                    "decoded");
        }
        if (params.fec_max_symbols_num == 0) {
            ...
            conn->fec_neg_fail_reason |= XQC_REMOTE_PARAM_ERR;
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|quic_fec|remote_setting's max_symbol_number is zero");
        }
    }
}

/*
 * RFC 9000 Section 7.4.1: after the server accepted early data it MUST NOT
 * reduce these transport parameters below the values the client remembered:
 ...
 * conn->remote_settings still holds the remembered (0-RTT) values set by
 * xqc_conn_set_early_remote_transport_params; the new 1-RTT values are in
 * `params`.  Returns XQC_OK when nothing was reduced.
 */
xqc_int_t
xqc_conn_check_0rtt_reduced_params(xqc_connection_t *conn,
    const xqc_transport_params_t *params)
{
    const xqc_trans_settings_t *remembered = &conn->remote_settings;
...
xqc_conn_tls_transport_params_cb(const uint8_t *tp, size_t len, void *user_data)
{
    ...
    xqc_transport_params_type_t tp_type =
        (conn->conn_type == XQC_CONN_TYPE_CLIENT ? XQC_TP_TYPE_ENCRYPTED_EXTENSIONS
    ...
    {
        if (xqc_conn_check_0rtt_reduced_params(conn, &params) != XQC_OK) {
    ...
    if (params.stateless_reset_token_present) {
        ...
        if (NULL != node) {
            ...
            xqc_log(conn->log, XQC_LOG_INFO,
                    "|store sr_token with cid: %s"
                    "|token:%s",
                    xqc_dcid_str(conn->engine, &cid_node->cid),
                    xqc_sr_token_str(conn->engine, params.stateless_reset_token));
            ...
            if (xqc_find_conns_hash(conn->engine->conns_hash_sr_token, conn,
                ...
                xqc_log(conn->log, XQC_LOG_ERROR, "|sr_token conflict:%s",
                        xqc_sr_token_str(conn->engine, cid_node->cid.sr_token));
            }
    ...
    if (ret != XQC_OK) {
        xqc_log(conn->log, XQC_LOG_ERROR, "|enable multipath error|ret:%d|", ret);
        XQC_CONN_ERR(conn, TRA_INTERNAL_ERROR);
        return;
    }
    ...
    if (conn->conn_settings.enable_encode_fec || conn->conn_settings.enable_decode_fec) {
    ...
    if (re_encode_local_tp_flag) {
        ...
        if (ret != XQC_OK) {
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|cannot reset local transport parameters while multipath version is "
                    "different");
            XQC_CONN_ERR(conn, TRA_INTERNAL_ERROR);
            return;
        }
        ...
        if (ret != XQC_OK) {
            xqc_log(conn->log, XQC_LOG_ERROR,
                    "|server tls update transport param error|ret:%d|", ret);
            XQC_CONN_ERR(conn, TRA_INTERNAL_ERROR);
            return;
        }
    }
...
xqc_int_t
xqc_conn_tls_crypto_data_cb(xqc_encrypt_level_t level, const uint8_t *data, size_t len,
                            void *user_data)
{
    xqc_connection_t *conn = (xqc_connection_t *)user_data;
    ...
    if (conn->crypto_data_total_len > XQC_CONN_MAX_CRYPTO_DATA_TOTAL_LEN) {
        xqc_log(conn->log, XQC_LOG_ERROR,
                "|crypto_data_total_len exceed limit|total_len:%ui|",
                conn->crypto_data_total_len);
...
xqc_int_t
xqc_conn_tls_cert_cb(const char *sni, void **chain, void **crt, void **key,
                     void *user_data)
{
    xqc_connection_t *conn = (xqc_connection_t *)user_data;

    if (conn->transport_cbs.conn_cert_cb) {
        return conn->transport_cbs.conn_cert_cb(sni, chain, crt, key, conn->user_data);
    }
...
void
xqc_conn_tls_msg_cb(int msg_type, const void *msg, size_t msg_len, void *user_data)
{
    xqc_connection_t *conn = (xqc_connection_t *)user_data;

    if (conn->transport_cbs.conn_ssl_msg_cb) {
        return conn->transport_cbs.conn_ssl_msg_cb(msg_type, msg, msg_len,
                                                   conn->user_data);
    }
}
        ...
        if (first_time_ack) {
            ...
            for (int i = 0; i < XQC_MAX_STREAM_FRAME_IN_PO; i++) {
                ...
                stream = xqc_find_stream_by_id(
                    packet_out->po_stream_frames[i].ps_stream_id, conn->streams_hash);
                if (stream != NULL) {
                    if (stream->stream_unacked_pkt == 0) {
                        xqc_log(conn->log, XQC_LOG_ERROR,
                                "|stream_unacked_pkt too small|");
                    ...
                    if (packet_out->po_stream_frames[i].ps_has_fin &&
                        stream->stream_stats.first_fin_ack_time == 0) {
                        stream->stream_stats.first_fin_ack_time =
                    ...
                    if (stream->stream_unacked_pkt == 0 &&
                        ...
                        xqc_log(conn->log, XQC_LOG_DEBUG,
                                "|stream enter DATA RECVD|stream_id:%d",
                                stream->stream_id);
                                     ...
                                     xqc_packet_out_t *packet_out, xqc_usec_t now)
{
    ...
    if (packet_out->po_frame_types &
        (XQC_FRAME_BIT_STREAM | XQC_FRAME_BIT_RESET_STREAM)) {
            ...
            if (stream[stream_cnt]) {
                ...
                if (j == stream_cnt) {
                    if (packet_out->po_sched_cwnd_blk_ts) {
                        stream[stream_cnt]->stream_stats.sched_cwnd_blk_duration +=
                            now - packet_out->po_sched_cwnd_blk_ts;
                        stream[stream_cnt]->stream_stats.sched_cwnd_blk_cnt++;
                    }
                    if (packet_out->po_send_cwnd_blk_ts) {
                        stream[stream_cnt]->stream_stats.send_cwnd_blk_duration +=
                            now - packet_out->po_send_cwnd_blk_ts;
                        stream[stream_cnt]->stream_stats.send_cwnd_blk_cnt++;
                    }
                    if (packet_out->po_send_pacing_blk_ts) {
                        stream[stream_cnt]->stream_stats.send_pacing_blk_duration +=
                            now - packet_out->po_send_pacing_blk_ts;
                    ...
                    stream[stream_cnt]->stream_stats.max_pto_backoff =
                        xqc_max(stream[stream_cnt]->stream_stats.max_pto_backoff,
                                ctl->ctl_pto_count);
                    stream_cnt++;
                }
            }
...
void
xqc_conn_check_app_limit(xqc_connection_t *conn)
{
    xqc_path_ctx_t *path = NULL;
    xqc_list_head_t *pos, *next;
    xqc_list_for_each_safe(pos, next, &conn->conn_paths_list)
...
xqc_conn_send_path_challenge(xqc_connection_t *conn, xqc_path_ctx_t *path)
{
    ...
    if (NULL == conn->transport_cbs.write_socket_ex) {
        xqc_log(conn->log, XQC_LOG_WARN,
                "|write_socket_ex not registered while sending PATH_CHALLENGE");
        return XQC_ERROR;
    }
    ...
    if (ret != XQC_OK) {
        xqc_log(conn->log, XQC_LOG_ERROR, "|xqc_generate_path_challenge_data error|%d|",
                ret);
        return ret;
    }
    ...
    sent = conn->transport_cbs.write_socket_ex(
        path->path_id, conn->enc_pkt, conn->enc_pkt_len,
        (struct sockaddr *)path->rebinding_addr, path->rebinding_addrlen,
        xqc_conn_get_user_data(conn));
    ...
    if (sent != conn->enc_pkt_len) {
        xqc_log(
            conn->log, XQC_LOG_ERROR,
            "|write_socket "
            "error|conn:%p|pkt_num:%ui|size:%ud|sent:%z|pkt_type:%s|frame:%s|now:%ui|",
        ...
        xqc_log(conn->log, XQC_LOG_INFO,
                "|<==|conn:%p|pkt_num:%ui|size:%ud|sent:%z|pkt_type:%s|frame:%s|inflight:"
                "%ud|now:%ui|",
                conn, packet_out->po_pkt.pkt_num, packet_out->po_used_size, sent,
                xqc_pkt_type_2_str(packet_out->po_pkt.pkt_type),
                xqc_frame_type_2_str(conn->engine, packet_out->po_frame_types),
    ...
    xqc_send_queue_remove_send(&packet_out->po_list);
    xqc_send_queue_insert_free(packet_out, &conn->conn_send_queue->sndq_free_packets,
...
xqc_conn_get_unscheduled_bytes(xqc_connection_t *conn)
{
    ...
    if (unsent_bytes_in_sndq >= scheduled_bytes) {
        ...
        xqc_log(conn->log, XQC_LOG_ERROR,
                "|scheduled_bytes_more_than_unsent_bytes|"
...
xqc_conn_unset_pkt_filter_callback(xqc_connection_t *conn)
{
    if (conn) {
        ...
        xqc_log(conn->log, XQC_LOG_INFO,
                "|conn unset pkt filter callback, will"
                "use write_socket again");
    }
}

int
xqc_conn_buff_0rtt_datagram(xqc_connection_t *conn, void *data, size_t data_len,
                            uint64_t dgram_id, xqc_data_qos_level_t qos_level)
{
    xqc_datagram_0rtt_buffer_t *buffer =
        xqc_datagram_create_0rtt_buffer(data, data_len, dgram_id, qos_level);
    if (buffer == NULL) {
        return -XQC_EMALLOC;
    }
...
xqc_gp_timer_id_t
xqc_conn_register_gp_timer(xqc_connection_t *conn, char *timer_name,
                           xqc_gp_timer_timeout_pt cb, void *user_data)
{
    return xqc_timer_register_gp_timer(&conn->conn_timer_manager, timer_name, cb,
                                       user_data);
}
...
{
    return xqc_timer_gp_timer_get_info(&conn->conn_timer_manager, gp_timer_id, is_set,
                                       expire_time);
}
                                         ...
                                         xqc_conn_public_local_trans_settings_t *settings)
{
    ...
    if (settings->max_datagram_frame_size !=
        conn->local_settings.max_datagram_frame_size) {
 ...
 */
void
xqc_conn_set_public_remote_trans_settings(
    xqc_connection_t *conn, xqc_conn_public_remote_trans_settings_t *settings)
{
...
xqc_int_t
xqc_conn_handle_stateless_reset(xqc_connection_t *conn, const uint8_t *sr_token)
{
    xqc_int_t ret;
    int res;
    xqc_list_head_t *pos, *next;
    ...
    xqc_list_for_each_safe(pos_set, next_set, &conn->dcid_set.cid_set_list)
    {
        ...
        xqc_list_for_each_safe(pos, next, &inner_set->cid_list)
        {
            ...
            res = xqc_memcmp(sr_token, cid->cid.sr_token, XQC_STATELESS_RESET_TOKENLEN);
            if (0 == res) {
                xqc_log(conn->log, XQC_LOG_INFO,
                        "|====>|receive stateless reset"
                        "|cid:%s|sr_token:%s|",
...
xqc_conn_available_paths(xqc_engine_t *engine, const xqc_cid_t *cid)
{
    ...
    xqc_log(conn->log, XQC_LOG_DEBUG, "|xqc_conn_available_paths|%" PRId32 "|",
            available_paths);
    return available_paths;
}
...
xqc_int_t
xqc_conn_handle_deprecated_stateless_reset(xqc_connection_t *conn, const xqc_cid_t *scid)
{
    if (NULL == conn) {
        return -XQC_EPARAM;
    }
