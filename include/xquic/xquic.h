 ...
 */
typedef enum { XQC_ENGINE_SERVER = 0, XQC_ENGINE_CLIENT = 1 } xqc_engine_type_t;
...
/**
 * @brief engine secret log callback. will only be effective when build with
 * XQC_PRINT_SECRET
 *
 ...
 * QUIC packets
 */
typedef void (*xqc_eng_keylog_pt)(const xqc_cid_t *scid, const char *line,
...
typedef struct xqc_log_callbacks_s {
     ...
     */
    void (*xqc_log_write_err)(xqc_log_level_t lvl, const void *buf, size_t size,
                              void *engine_user_data);
     ...
     */
    void (*xqc_log_write_stat)(xqc_log_level_t lvl, const void *buf, size_t size,
                               void *engine_user_data);
     ...
     */
    void (*xqc_qlog_event_write)(qlog_event_importance_t imp, const void *buf,
...
/**
 * @brief connection refused callback. corresponding to xqc_server_accept_pt callback
...
/**
 * @brief engine can't find connection related to input udp packet, and return a
 ...
 */
typedef void (*xqc_save_string_pt)(const char *data, size_t data_len,
                                   void *conn_user_data);
 ...
 */
typedef void (*xqc_conn_update_cid_notify_pt)(xqc_connection_t *conn,
 ...
 * @param conn_user_data connection level user_data
 */
typedef void (*xqc_conn_peer_addr_changed_nofity_pt)(xqc_connection_t *conn,
                                                     void *conn_user_data);
 ...
 * @param conn_user_data connection level user_data
 */
typedef void (*xqc_path_peer_addr_changed_nofity_pt)(xqc_connection_t *conn,
                                                     uint64_t path_id,
                                                     void *conn_user_data);
...
/**
 * @brief sendmmsg callback function. the implementation of this shall send data with
 * sendmmsg
 *
 ...
 */
typedef ssize_t (*xqc_conn_pkt_filter_callback_pt)(const unsigned char *buf, size_t size,
                                                   const struct sockaddr *peer_addr,
                                                   socklen_t peer_addrlen,
                                                   void *cb_user_data);
 ...
 */
typedef xqc_int_t (*xqc_conn_cert_cb_pt)(const char *sni, void **chain, void **crt,
                                         void **key, void *user_data);

typedef void (*xqc_conn_ssl_msg_cb_pt)(int msg_type, const void *msg, size_t msg_len,
                                       void *user_data);
 ...
 * @param conn_user_data user_data of connection
 */
typedef int (*xqc_path_created_notify_pt)(xqc_connection_t *conn, const xqc_cid_t *scid,
 ...
 */
typedef ssize_t (*xqc_socket_write_ex_pt)(uint64_t path_id, const unsigned char *buf,
 ...
 * when send a reset packet
 */
typedef ssize_t (*xqc_send_mmsg_ex_pt)(uint64_t path_id, const struct iovec *msg_iov,
                                       unsigned int vlen,
                                       const struct sockaddr *peer_addr,
                                       socklen_t peer_addrlen, void *conn_user_data);
 ...
 * @return 0 for success, -1 for failure
 */
typedef void (*xqc_stream_closing_notify_pt)(xqc_stream_t *stream, xqc_int_t err_code,
                                             void *strm_user_data);
 ...
 */
typedef void (*xqc_datagram_read_notify_pt)(xqc_connection_t *conn, void *user_data,
 ...
 */
typedef void (*xqc_datagram_acked_notify_pt)(xqc_connection_t *conn, uint64_t dgram_id,
                                             void *user_data);
 ...
 */
typedef void (*xqc_datagram_mss_updated_notify_pt)(xqc_connection_t *conn, size_t mss,
                                                   void *user_data);


/**
 * @brief tranport callback functions are more related to attributes of QUIC [Transport]
 * but not ALPN.
 *
...
typedef struct xqc_transport_callbacks_s {
    ...
    /**
     * path remove callback function. REQUIRED both for client and server if multi-path is
     * needed
     */
...
typedef struct xqc_datagram_callbacks_s {
    /**
     * @brief datagram read callback function. REQUIRED for both client and server if they
    ...
    /**
     * @brief datagram write callback function. REQUIRED for both client and server if
...
/**
 * @brief connection and stream callbacks for QUIC level, Application-Layer-Protocol shall
...
typedef enum { XQC_FEC_RANDOM_TBL = 0, XQC_FEC_BURST_TBL = 1 } xqc_fec_tbl_mode_e;
...
typedef struct xqc_congestion_control_callback_s {
    ...
    /** Callback on connection initialization, support for passing in congestion algorithm
     * parameters */
    void (*xqc_cong_ctl_init)(void *cong_ctl, xqc_send_ctl_t *ctl_ctx,
                              xqc_cc_params_t cc_params);

    /** Callback when packet loss is detected, reduce congestion window according to
     * algorithm */
    ...
    /** Callback when all packets are detected as lost within 1-RTT, reset the congestion
     * window */
...
typedef struct xqc_scheduler_callback_s {
    ...
    xqc_path_ctx_t *(*xqc_scheduler_get_path)(void *scheduler, xqc_connection_t *conn,
                                              xqc_packet_out_t *packet_out,
                                              int check_cwnd, int reinject,
                                              xqc_bool_t *cc_blocked);

    void (*xqc_scheduler_handle_path_event)(void *scheduler, xqc_path_ctx_t *path,
                                            xqc_scheduler_path_event_t event,
                                            void *event_arg);

    void (*xqc_scheduler_handle_conn_event)(void *scheduler, xqc_connection_t *conn,
...
typedef struct xqc_reinj_ctl_callback_s {
    ...
    xqc_bool_t (*xqc_reinj_ctl_can_reinject)(void *reinj_ctl, xqc_packet_out_t *po,
...
typedef struct xqc_fec_code_callback_s {
    ...
    xqc_int_t (*xqc_fec_encode)(xqc_connection_t *conn, unsigned char *unit_data,
                                size_t un_size, unsigned char **outputs,
                                uint8_t fec_bm_mode);
    xqc_int_t (*xqc_fec_decode)(xqc_connection_t *conn,
                                unsigned char **recovered_symbols_buff, size_t *size,
                                xqc_int_t block_idx);
    xqc_int_t (*xqc_fec_decode_one)(xqc_connection_t *conn,
                                    unsigned char *recovered_symbols_buff,
...
typedef struct xqc_config_s {
    ...
    /**
     * only for server, whether server will negotiate cid with client. non-zero for
    ...
    /** for warning when the number of elements in one bucket exceeds the value of
...
typedef struct xqc_engine_callback_s {
    ...
    /** get realtime timestamp callback function. if not set, xquic will get timestamp
       with inner function xqc_now, which relies on gettimeofday */
    xqc_timestamp_pt realtime_ts;

    /** get monotonic increasing timestamp callback function. if not set, xquic will get
...
typedef struct xqc_conn_settings_s {
    ...
    /**
     * @brief initial pto duration (us). Default: 0 (use the internal default value --
     * 3xinitial_rtt)
     *
     */
...
#define XQC_PATH_HARD_CAP \
    256 /* defensive local hard max; spec §2.1 allows up to 2^32-1 */
...
/**
 * @brief register alpn and connection and stream callbacks. user can implement his own
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_engine_register_alpn(xqc_engine_t *engine, const char *alpn,
                                   size_t alpn_len, xqc_app_proto_callbacks_t *ap_cbs,
                                   void *alp_ctx);
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_engine_unregister_alpn(xqc_engine_t *engine, const char *alpn,
                                     size_t alpn_len);
...
XQC_EXPORT_PUBLIC_API
xqc_int_t
xqc_engine_packet_process(xqc_engine_t *engine, const unsigned char *packet_in_buf,
...
/**
 * @brief Process all connections, application implements MUST call this function in timer
 * callback
 */
XQC_EXPORT_PUBLIC_API
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_engine_get_default_config(xqc_config_t *config,
...
/**
 * @brief Set server's connection settings. it can be called anytime. settings will take
 * effect on new created connections
 */
XQC_EXPORT_PUBLIC_API
void xqc_server_set_conn_settings(xqc_engine_t *engine,
...
XQC_EXPORT_PUBLIC_API
xqc_connection_t *xqc_engine_get_conn_by_scid(xqc_engine_t *engine, const xqc_cid_t *cid);
...
/**
 * Send CONNECTION_CLOSE to peer, conn_close_notify will callback when connection
 * destroyed
...
/**
 * @brief set application-layer-protocol user_data to xqc_connection_t. which will be used
...
xqc_int_t xqc_conn_get_local_addr(xqc_connection_t *conn, struct sockaddr *addr,
                                  socklen_t addr_cap, socklen_t *local_addr_len);
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_conn_send_ping(xqc_engine_t *engine, const xqc_cid_t *cid,
                             void *ping_user_data);
 ...
 */
XQC_EXPORT_PUBLIC_API
void xqc_conn_set_public_local_trans_settings(
    xqc_connection_t *conn, xqc_conn_public_local_trans_settings_t *settings);
 ...
 */
XQC_EXPORT_PUBLIC_API
void xqc_conn_set_public_remote_trans_settings(
 ...
 */
XQC_EXPORT_PUBLIC_API
ssize_t xqc_stream_recv(xqc_stream_t *stream, unsigned char *recv_buf,
 ...
 */
XQC_EXPORT_PUBLIC_API
ssize_t xqc_stream_send(xqc_stream_t *stream, unsigned char *send_data,
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_datagram_send(xqc_connection_t *conn, void *data, size_t data_len,
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_datagram_send_multiple(xqc_connection_t *conn, struct iovec *iov,
                                     uint64_t *dgram_id_list, size_t iov_size,
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_datagram_send_on_path(xqc_connection_t *conn, void *data, size_t data_len,
                                    uint64_t *dgram_id, xqc_data_qos_level_t qos_level,
                                    uint64_t path_id);
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_conn_close_path(xqc_engine_t *engine, const xqc_cid_t *cid,
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_conn_mark_path_standby(xqc_engine_t *engine, const xqc_cid_t *cid,
                                     uint64_t path_id);
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_conn_mark_path_available(xqc_engine_t *engine, const xqc_cid_t *cid,
                                       uint64_t path_id);
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_conn_mark_path_frozen(xqc_engine_t *engine, const xqc_cid_t *cid,
                                    uint64_t path_id);
 ...
 */
XQC_EXPORT_PUBLIC_API xqc_int_t xqc_conn_available_paths(xqc_engine_t *engine,
                                                         const xqc_cid_t *cid);
 ...
 */
XQC_EXPORT_PUBLIC_API
xqc_int_t xqc_lb_cid_encryption(uint8_t *cid_buf, size_t enc_len, uint8_t *out_buf,
                                size_t out_buf_len, uint8_t *lb_cid_key,
                                size_t lb_cid_key_len, xqc_engine_t *engine);
